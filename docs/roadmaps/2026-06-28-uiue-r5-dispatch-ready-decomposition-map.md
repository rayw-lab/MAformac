...
| Row | Reason to wait |
|---|---|
| `C005` | D9 covers only the current local mock executor/store write path. Production runtime adapter ownership remains future proof. |
...
| `C061` | D9 partially covers already-state no-double-write. Retry/full runtime adapter idempotency remains future proof. |
