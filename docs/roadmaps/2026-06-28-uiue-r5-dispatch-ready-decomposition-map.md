...
| Row | Reason to wait |
|---|---|
| `C005` | D12 Runtime Adapter V0 covers adapter-owned mock write path under main local/unit proof (`451c699`). Production runtime wiring, durable execution ownership, mobile/true-device/live proof, and UIUE merge remain future work. |
...
| `C061` | D12 Runtime Adapter V0 covers stable command identity, deterministic request fingerprint, in-memory successful ledger, retry replay no double-write, conflict fail-closed, and failed-command no fake-success ledger under main local/unit proof (`451c699`). Persistent retry ledger, production runtime integration, mobile/true-device/live proof, and UIUE merge remain future work. |
