...
**推进式（重大派单范式，grill+红队成对）**：
1. **D0 立项 grill**（可立即，不占训练资源）：把 UIUE 已决 recall 清单（防重拍 U/V/D 系）+ macOS 适配未决议题（布局/交互语义/演示编排/性能）做成模块 grill 骨架——与 Line C C1 grill 同批走「模块 grill 骨架弹药范式」（宪法 §21）。
2. **D1 UIUE macOS 适配开发**：commander 亲笔专场（磊哥说「好好干」= 预期高投入窗口），5 Gate 视觉验收 + 还原实查。
3. **D2 runtime 接线**：S10 verdict 后接新 adapter；golden-run 脚本（汇合点门 4）在此实装。
4. **D3 演示编排与彩排**：demo-scenarios 全场景 + 兜底话术分族（C3 供给）+ 5 分钟现场彩排门。
...
───────────────────────────┬──────────────────────────┬──────────────────────────┬──────────────────────────┬─────────────
Line B  [暂停待窗口]────────→ S8 重启跑完 ─────────────→ S9 三臂 eval → S10 verdict ─→(供给 C2/D2)
Line C  C1 grill 骨架+矩阵──→ (文档线不占资源, 继续)───→ C2 扩挂载(按已拍三档表)────→ C3 兜底链路验收──────────→┐
Line D  D0 立项 grill───────→ (同上)───────────────────→ D2 runtime 接新 adapter───→ D1 UIUE 专场 + D3 彩排──→ 五门全绿
...
- Line C：`runs/2026-07-07-ma-opt-refactor/out/task3-capability-iceberg-teardown.md`（弹药）+ `runs/2026-07-07-ma-opt-refactor/grill-skeleton/c1-design-drafts.md`（C1 设计草稿）
- Line D：UIUE 已决 = `docs/grill-tournament/grill-decisions-master.md`（U1-31 挂 Q30-38）+ memory `feedback-uiue-no-codex`（主刀铁律）；D0 grill 骨架落 run dir（立项后建）
