...
- **S9 预备件三 checker 全落位（v5 晚间刷新）**：stop_event / mount-validity TDD 绿；train-eval exposure checker 二轮修复（语序不敏感层）后**已落仓 `scripts/check_train_eval_exposure.py`（commit `c4dfb247`）+ S10 模板第 7 门已实装**——判别力田间四连证（v2 addendum 预检抓 1 撞/v3 零撞/J2 全量扫出 8/J3 复扫 FP dismiss）。
- **S9 holdout 已 FROZEN（v5 晚间刷新，D-124→D-127 战役链闭环）**：D-122 Opus raw（N=64）→ J1 47/64 → 修复战役（FP 三方双盲剔10救3 / register 修4 / 补行两轮各过 exposure 预检）→ J2 全量 56/64（增量对句推翻 2 救回，commander 亲裁）→ J3 58/61 → args 修正 → **J4 全绿 61/61** → **sha256 `77853caea4598f334fb4…` 冻结**，61 行四桶 33/9/10/9 全过门，canonical=`s8-gates/s9-eval-freeze/holdout/eval-holdout.jsonl`（freeze-receipt 带 v1→v2→v3 lineage）。S9 弹药就绪，只等 S8 adapter。
...
- Line C：`runs/2026-07-07-ma-opt-refactor/out/task3-capability-iceberg-teardown.md`（弹药）+ `runs/2026-07-07-ma-opt-refactor/out/c1-design-drafts.md`（C1 设计草稿；🔴v4 audit P1-3 修正——原误写 grill-skeleton/ 目录）+ **`runs/2026-07-08-daywork/capability-matrix-v3.md`（D-123 内容 SSOT，D-133 使 C1 P0 38/38 ⭐B RATIFIED：120=10族×4 value_shape×3 register；主分类账 unmounted=36 / fastpath=82 / default=1 / conditional=1，safety=0。v1 draft 的「拒识36/unknown7」口径已被审计推翻，留档勿引；本拍不授权 mounted 1→N）**
