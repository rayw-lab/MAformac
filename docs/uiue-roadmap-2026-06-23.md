...
- 🔴 **D8.3 L3+ 思考链路（假 COT）= orb `think` 态**（2026-06-24 拍）：orb TimelineView 四态 idle/`think`/speak/listen 的 **think 态承载** L3+ 复杂推理演出（「我有点困了」→「思考中…→调用中…→为您开窗通风」）。**对话级 orb 演出，非卡片态（DemoVisualState 7 态不动）**。前端假 COT + 场景宏确定性查表（demo，范式§175）/ LoRA 学真 COT（后续）。可视化三层路由（L1 秒回不演 / L3+ 演思考）。
- 🔴 **E 组 grill 收口（E0-E8，思考链路 orb + DA0 全契约，权威 = grill-master §3 E 组）**：① **事件驱动（非计时）**：orb think analyzing 掩盖后端 → **后端 10 族卡片开始 changing 跳动（`cardsDidStartChanging` 事件）= 结束信号** → handoff(orb speak + TTS) + 卡片 stagger 级联并行（3s 虚数非固定）② **orb 实现 E1**：自建多层 MeshGradient(底深空青紫慢漂+中think脉冲+顶高光斑)+breathing spring+hanlin LoadingGradientText 思考文字流光+可选 Canvas 粒子（零 metasidd[19月stale]/零 Inferno[Metal 撞 U30]）③ **think 两语义 E8**：思考链路 think=掩盖后端动态 vs 安全拒识 think=演出固定1.0s ④ **触发 E3**：Core 新建 SceneMacroMatcher 关键词匹配→`triggered_macro_id` 信号→UIUE 接信号(不判语义)+手动 force(长按orb1.5s+#if DEMO_MODE)+L1 listen 100ms微亮+barge-in 打断连续L1(U21) ⑤ **场景宏 E4**：首批4宏(上车迎宾/离车收尾/雨天关窗天窗/困了)+narration 2字段(analyzing+readback)+trigger_tags 10同义词，归 Core scenario-macros.yaml ⑥ **DA0 deny→态 E5-E7**：store `applyGuardBlock(key:String,态,reason)` 非throw + reason→态映射(safety→unsafe/族外→blocked_hard/confirm→clarify) + 统一 `reasons:[String:String]` @Observable map(cell 无字段符 DA1) + `systemFailure` 真异常分流。**关键发现**：主流程 DemoFastPathGuard 占位(只演打开空调)，安全拒识/族外 deny 体系要扩 guard 接 C3。⑦ voice 音频/炸场 DEFERRED。
- **多卡时序守 D1 级联**（D8.5）：「困了→多动作」走 stagger 220ms 快速渐次 + MAX_CONCURRENT_HIGHLIGHTS=1（不同时炸，已 grill 拍）。LoRA 超时（demo 场景宏即时无此问题 = E2 事件驱动后端 ready 即切；后续真 LoRA 用 γ = **事件驱动 + 超时阈值降级规则兜底**，非 3s 固定演出，E 组 E2 事件驱动级联修正）。
