...
> 🔴 **A2 链路 B 仍在审计修复循环中（未结束，2026-06-24 实况纠正）**：A2 = code-only 范式对齐 generic frame→D-domain（主工作树 `a2/migrate-d-domain-tool-surface`），但**还在 build+audit-fix 循环**，未收口。「派单就绪/盘点完成」≠「重构跑完 + swift test/make verify 绿 + 审计收敛」。A2 绿后才并 main → 才起训练线。**训练线 = 三步（DEFERRED 解冻，非 A2 完直接训）**：① C5 数据生成（D-domain 四类自然中文语料）② C5 实际重训（守 rank16Mainline+LR1e-4）③ C6 四层评测（candidate vs base 10/23 不退化）。UIUE 链路 A 与 A2 **两 worktree 并行零撞车**，不卡在 A2 审计循环上。
...
主仓 /Users/wanglei/workspace/MAformac
  ├─ 主工作树（cwd）        → 分支 a2/migrate-d-domain-tool-surface  ← 链路 B（A2 仍在审计修复循环，未结束；绿后才起训练线）
...
| 阶段 | 动作 | 时机 |
|---|---|---|
| **T1 A2 落地** | `a2/migrate-d-domain-tool-surface` → PR → **merge main**（code-only，swift test + make verify 绿）| 🔴 **A2 审计修复循环收敛 + 绿后**才并 main（现仍在审计循环，未结束）|
...
**A2 还在审计修复循环（未结束）→ 收敛 + 绿后才 PR 回 main → 才起训练线（C5 数据生成 → C5 重训 → C6 评测三步，非 A2 完直接训）。UIUE 不卡在 A2 审计循环上，留 uiue worktree 继续并行开发；某 Phase 要 A2 产物时（rebase main 拿 D-domain）才依赖 A2 已并 main。最终 demo 在 main 上三者（A2 surface + UIUE 7 态 + LoRA 权重）汇合，不让两链路互相直接 merge。**
