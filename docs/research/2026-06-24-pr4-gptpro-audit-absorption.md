...
- GitHub Actions 跑 `make verify-ci`：verify-refs、cross-section、surface、default-scope gates、diff、python tests、swift test，加 `git diff --check`。它不跑 `verify-source`，也不跑 `regen`，因为 `gen_c1.py` 仍读取本机 raw source snapshots；Swift 中依赖本机 HuggingFace tokenizer cache 的 fixture 在 CI 缺失时显式 `XCTSkip`，本地 `make verify-all` 仍会在 fixture 存在时跑真路径。
