...
| 6 | 元扳机 hook | 顶层 `scripts/hooks/` | Q1-5+A1-4 | 🟡 verified(piloting) | syntax OK + 子 spec + 沙盒 8 绿(C5 调研→skip_research✅) |
...
- 当前态：4 session / C5 调研 Agent 误检已修（沙盒 skip_research✅）/ 子 spec 已建 / 挂 uiue worktree `.claude/settings.local.json`(gitignored)。
- 待：≥5 session 主观不烦 → 迁用户级（一并做项7 + 摘 token-threshold-hook）。
...
- 🔴 **整体剩 2 个未 landed**：① hook 迁用户级（试点 ≥5 会话门，差 1）② 项目宪法 3 条（main session apply patch）。其余 12 项收口。
