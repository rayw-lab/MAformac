...
| `2026-06-22-raw-vault-5-evidence-P.md`(原 🔴emoji 名已规范化;§6 放宽 private 仓内部调研档入仓) | C5 recovery **ε=方案 P** 的 RAW vault 一手工程证据(FC手册:248 Car→U播报/端状态协议推荐层/AIOS分层/V1.0可追责);CC 辩证 catch 证据3过度引申(TTS归一化P/Q都需要)+混合架构漏读(readback走端renderer/clarify走模型) | **C5 recovery readback 实装**(删 `:1038`+readback单列gate);`grill-decisions ε` 决策一手源 |
...
| 🔴 `2026-06-22-top-fc-skill-table-teardown/README.md` | **真实座舱 FC 契约 + 交付手册一手 teardown（某车厂，只读不入仓）**：ground-truth = **D-domain 极致密集版（2045 具名工具 / domain16→service-group236→tool2045），value-form 编码进工具名（_to_max/_to_number/_to_exp/raise_by），不用统一 B-frame**；value 四件套 `{ref,direct,offset,type}` + SPOT/EXP 与 MAformac 一字不差；FC 只产 (name,params)+确定性协议转换适配器汇入既有 DS/DM；label=自由/指令、clarifyTag:implicit、意图收缩模型；测试集 ~10.5k 单轮+932 多轮按工具名匹配；指标 模糊≥90/自由≥80/整体≥85/误吸≤5%/≤2400ms;安全二次确认设备清单 | **🔴 B-frame vs D-domain 范式决策 grill**（直接 implication §7：守 D-domain + codegen 全量 + service-group 分层 + 端侧挂 demo 子集，B-frame 已被真实座舱否决）；C5 数据分层/C6 bench 口径 |
| 🔴 `2026-06-22-a2-codebase-audit/README.md`(+lens1-8+codex-checks+INDEX) | **A2 重型重构代码盘点（ultracode 8 finder + 综合官 / 9 agent / 主线程亲核）**：C1→C2→C3→C5→C6 全链路不对齐全表(16 处 file:line)+ A2 范围(~14-16 文件/1500-2500 行/6 步依赖序)+ 复用/重写/新建三分类 + Swift codegen 选型(守 Python codegen **不引新框架**)+ mlx/端侧受限解码(mlx-swift-structured vendor)+ 硬编码避免机制 + 16 pre-mortem + 8 grill 弹药；🔴 **数字口径坐实**(device 191/intent **562**(磊哥 2026-06-23 终拍,旧 534/2086 废)/工具数未拍,generated 旁路 223 过期,562=intent 非工具数)；transcript 最一手归档仓外 raw `-transcripts/` | **A2 派单前必读**；C1-C6 代码重构起手 |
| `2026-06-25-a2-execution/a2-final-jsonl-reviewed-closeout-report.md` | **UIUE A-2 10+小时长跑 JSONL-reviewed closeout**：727 行最终报告，回看 live session JSONL/lessons/process docs/receipts/tasks/coverage；结论为 **PARTIAL overall**，Phase 0/1/3/4/5/6 只在各自 local/unit/simulator/mock scope 收口，Phase 2 `8.A/8.C2`、full Phase 7 visual closeout、true-device/mainline/product V-PASS 仍 open。 | **UIUE A-2 恢复/交接/claim-vs-reality 审计起手**；先读本报告 + `a2-phase-closeout-receipt.md`，再读对应 phase receipt |
| `2026-06-25-cc-harness-mechanism-review/INDEX.md` | **CC harness 管控机制复盘 + 业界对标任务索引**：5 路 teardown（grill 全流程 / 框架联动 / 通用管控 / 业界对标 / 本机现状）+ 机制分层落位；关联 **loop review goals 派单实施文件** 见下一行（raw 仓外一手派单，本索引只挂路径不搬正文）。 | **长跑 / goal / 派单 / grill 流程机制沉淀前必读**；回溯「执行切片 vs 派单全貌」、per-turn drift hook、derived-tracking 骨架预留时读 |
| 🔗 `~/workspace/raw/05-Projects/MAformac/dispatches/2026-06-22-harness-enforce-audit-implementation-dispatch.md` | **loop review goals 派单实施文件**：harness enforce / audit implementation 的 raw 仓外一手派单；用于追溯 hook/enforce 落地、4 轮审计循环、3×P1+2×P2 修复与 lessons，正文不搬入仓。 | **/goal / loop review / 派单执行回溯前必读**；确认执行切片是否覆盖派单全貌、before/after hook、derived-tracking 回写 |
| `2026-06-24-pr4-gptpro-audit-absorption.md` | **PR #4 两份 GPT Pro 审计吸收记录**：P0=head-bound receipt + CI/status 合并门；P1=state-applier fail-closed / C6 scopeOrigin 单源 / C6-C3 readback drift / C5 全设备 parity / unknown state key；并记录 UIUE 边界（默认 scope 非完全省略、ContentView 移交 UIUE、接口保护）。 | **default_scope apply closeout / PR #4 merge 前必读**；后续 UIUE rebase 和 retrain-c5/rebuild-c6 前确认哪些 finding 已修、哪些 deferred。 |
| `2026-06-24-pr4-gptpro-architecture-absorption.md` | **PR #4 GPT Pro 下半段架构建议吸收**：辩证吸收 `ScopedStateKey / TargetResolution / PlannedEffect / ContractReplayEngine / PresentationAdapter / C2-derived C5 scope / ContractBundleFingerprint / PR lane split`，区分已落地止血、后续 runtime spine、UIUE、rebuild-c6、retrain-c5、evidence governance。 | **后续 default_scope 架构债 / rebuild-c6 / retrain-c5 / UIUE presentation 前必读**；避免把 PR #4 止血版误当完整 contract spine，也避免一次性大爆炸重构。 |
| ⭐ `2026-07-08-macos-uiue-methodology/README.md`(+lens01-07+INDEX+transcripts) | **macOS App 前端 UIUE 开发方法论调研**(workflow 14 agent:6 联网 finder+1 本地基线+逐路引用核验+综合官,全 opus/medium):设计原则(HIG/Liquid Glass 铁律)/流程(design system 7 步/token 三层 SSOT/代码即原型)/工程(纯 MV 去 ViewModel/token enum/Preview 驱动/感知性能 Linear 范式)/评审(Nielsen 0-4 severity+macOS 专属维度+WCAG 门)/AI 辅助(截图对照闭环+编译门,幻觉/漂移对策);带 8 条 MAformac 适配建议 P0/P1/P2 分级 + 4 grill 弹药;3 处假引用已核验剔除 | **Line D UIUE 前置化 / D1 UIUE 专场 / tokens SSOT 落地 / Line D 验收门设计前必读** |
| 🔴 `2026-07-21-phase1-remediation-independent-audit/README.md`（+7 lens+INDEX+transcript 指针） | **Phase 0/1a/1b 整改独立审计**：亲跑 core E2E、full Swift、anti-placebo、UI Make 门、iOS26 模拟器 XCUITest 与 GitHub required checks；裁决整改 `NO-GO`。坐实 UI 前台失败、UI 判分器吞失败、test07 缩门、COR-4 typed no-action 未闭合、2112 行活跃测试退出、COR-2/7/8 与 payload schema 漂移；同时撤销 UI test 不存在、verify-e2e 未进 CI、protected 本轮越界等外援误判。审计后九点已全量映射进 **Program WBS V10** 与 `PHASE1-APPENDFIX-IMPLEMENTATION-PLAN.md`，但未执行。 | **Program WBS V10 baseline / Phase1-AppendFix / 重做 Phase 1 验收 / 判断 handoff“闭环”真伪前必读**；最终事实以 README + INDEX 纠错账为准，计划存在不等于 AF-G0 通过。 |
