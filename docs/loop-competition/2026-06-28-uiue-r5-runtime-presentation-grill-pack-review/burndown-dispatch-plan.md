...
| ID | Original | P | Route | Action | Avg | Question | Next step |
|---|---|---|---|---|---:|---|---|
| C003 | RPB-03 | P1 | mainline_first | Keep | 21.7 | bridge 4 名和字段名必须由 mainline carrier/DTO 锁定。 | D15 disposition: `covered_by_D15_payload_contract_local_unit` for main-owned `RuntimePresentationPayload` schema/field carrier in commits `c212863`/`ab9a682`/`1d9b674`; UIUE consumer integration remains future D17. |
...
| C024 | RPB-24 | P1 | mainline_first | Keep | 20.5 | TraceEnvelope 最小字段和 redaction 需要锁。 | D15 disposition: `covered_by_D15_payload_contract_local_unit` for presentation-safe trace redaction, including trace identity/run/parent-span side-channel redaction; proof remains local/unit and not runtime/mobile. |
...
| C062 | CE-009 | P1 | mainline_first | Keep | 21.0 | snapshot 禁 raw model output, 只带 presentation-safe outcome。 | D15 disposition: `covered_by_D15_payload_contract_local_unit`; encoded payload negative tests cover raw model output, raw runtime store, training receipt, and adapter-private marker redaction. |
| C097 | CE-044 | P1 | mainline_first | Keep | 21.3 | mainline/UIUE proof class crosswalk。 | D15 disposition: `covered_by_D15_payload_contract_local_unit` for finite payload proof class and proof-cap non-claims. UIUE D17 must consume without proof promotion. |
| C138 | MC-003 | P1 | mainline_first | Keep | 21.3 | `isTerminal` 由结果类派生还是 runtime adapter 显式写入。 | D15 disposition: `covered_by_D15_payload_contract_local_unit`; payload carries stable `isTerminal` derived from existing `PresentationSnapshot`, not adapter private state. |
...
| C150 | MC-015 | P1 | mainline_first | Rewrite | 20.7 | 未知 `PresentationProofClass` JSON 是否所有 consumer 都 fail-closed。 | D15 disposition: `covered_by_D15_payload_contract_local_unit`; payload enum decoding fails closed for unknown schema/proof/reconciliation values in main tests. UIUE consumer fail-closed proof remains D17. |
