...
| ID | Original | P | Route | Action | Avg | Question | Next step |
|---|---|---|---|---|---:|---|---|
| C003 | RPB-03 | P1 | mainline_first | Keep | 21.7 | bridge 4 名和字段名必须由 mainline carrier/DTO 锁定。 | D15 disposition: `covered_by_D15_payload_contract_local_unit` for main-owned `RuntimePresentationPayload` schema/field carrier in commits `c212863`/`ab9a682`/`1d9b674`; D17 UIUE consumes only the stable payload schema/field names through `RuntimePresentationConsumerMapping` local/unit fail-closed tests in UIUE commit `f55a80e`. |
| C005 | RPB-05 | P1 | mainline_first | Keep | 21.5 | 写所有权: 触摸/事件走 executor 或 runtime adapter, 不直接写 store。 | D18 disposition: `covered_by_D18_local_durable_adapter_ledger` for main-owned local file-backed durable adapter replay on adapter-owned mock writes, strict unknown-key durable JSON fail-closed behavior, readback reconciliation, and private payload boundary verification in main commits `83c286c`/`3439c37`/`b6b92fd`/`b6a7937`. D19 UIUE guard rejects D18 durable/private names only; it does not consume durable rows. Production durable runtime wiring, mobile/true-device/live proof, UIUE runtime consumer proof, and UIUE merge remain future proof. Preserve original row. |
...
| C018 | RPB-18 | P1 | mainline_first | Keep | 20.3 | SceneMacroRegistry 属 Core config, 非 UIUE-only 隐藏 planner。 | D16 disposition: `covered_by_D16_core_config_scene_macro_registry_local_unit`; main commits `16860c8`/`d00023a` define OpenSpec + `SceneMacroRegistry` stable config/macro names and fail-closed tests. D17 UIUE commit `f55a80e` consumes those names by raw-name allow-list only; UIUE still owns no Core config truth. |
...
| C024 | RPB-24 | P1 | mainline_first | Keep | 20.5 | TraceEnvelope 最小字段和 redaction 需要锁。 | D15 disposition: `covered_by_D15_payload_contract_local_unit` for presentation-safe trace redaction, including trace identity/run/parent-span side-channel redaction; D17 UIUE maps only `traceEnvelope` as a stable presentation field and rejects private adapter/raw names. Proof remains local/unit and not runtime/mobile. |
...
| C052 | RPB-52 | P2 | mainline_first | Rewrite | 19.7 | force-state context 输入需 `#if DEMO_MODE` + bridge event + trace provenance。 | D16 disposition: `covered_by_D16_force_state_demo_debug_boundary_local_unit` after Gate3/Gate4R. Main commits `47c5e9c`/`ac1569f` require debug/demo isolation, demo-harness provenance, no `.customerFacing`, duplicate/empty fail-closed behavior, fixed `.localUnit` proof, and no external `DemoForceStateContext` Decodable/Codable construction. Production/runtime force-state remains future owner work. |
| C061 | CE-008 | P2 | mainline_first | Rewrite | 19.3 | retry/idempotency 规则: 重试不得二次写 state 或吞掉 no-op。 | D18 disposition: `covered_by_D18_local_durable_adapter_ledger` for cross-adapter retry replay, cross-pipeline C3 settled parent replay, fingerprint mismatch fail-closed, failure-not-success replay, readback drift fail-closed, and unknown durable JSON keys fail-closed in main commits `83c286c`/`3439c37`/`b6b92fd`. D15/D17 expose only presentation-safe reconciliation names; D19 UIUE rejects retry identity/ledger/durable private names. Production durable runtime integration, mobile/true-device/live proof, UIUE runtime consumer proof, and UIUE merge remain future. Preserve original row. |
| C062 | CE-009 | P1 | mainline_first | Keep | 21.0 | snapshot 禁 raw model output, 只带 presentation-safe outcome。 | D15 disposition: `covered_by_D15_payload_contract_local_unit`; encoded payload negative tests cover raw model output, raw runtime store, training receipt, and adapter-private marker redaction. D17 UIUE rejects `rawModelOutput`, `trainingReceipt`, `rawRuntimeStore`, `runtimeStore`, and adapter-private names through deny-list or unknown-field fail-closed verifier. |
| C097 | CE-044 | P1 | mainline_first | Keep | 21.3 | mainline/UIUE proof class crosswalk。 | D15 disposition: `covered_by_D15_payload_contract_local_unit` for finite payload proof class and proof-cap non-claims. D17 UIUE consumes D15 proof labels with fail-closed tests and keeps proof caps to local/unit/simulator_mock; no readiness display cap is promoted. |
| C138 | MC-003 | P1 | mainline_first | Keep | 21.3 | `isTerminal` 由结果类派生还是 runtime adapter 显式写入。 | D15 disposition: `covered_by_D15_payload_contract_local_unit`; payload carries stable `isTerminal` derived from existing `PresentationSnapshot`, not adapter private state. D17 UIUE consumes `isTerminal` only as a stable payload envelope field. |
...
| C150 | MC-015 | P1 | mainline_first | Rewrite | 20.7 | 未知 `PresentationProofClass` JSON 是否所有 consumer 都 fail-closed。 | D15 disposition: `covered_by_D15_payload_contract_local_unit`; payload enum decoding fails closed for unknown schema/proof/reconciliation values in main tests. D17 UIUE commit `f55a80e` adds local/unit fail-closed validators/tests for unknown schema, proof, reconciliation status/mismatch, config, macro, force-context, and presentation field names. |
