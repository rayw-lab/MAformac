...
| C005 | RPB-05 | P1 | mainline_first | Keep | 21.5 | 写所有权: 触摸/事件走 executor 或 runtime adapter, 不直接写 store。 | D13 disposition: `covered_by_D13_c3_runtime_adapter_local_unit` for C3-path adapter-owned mock write proof in main commit `612e0df`; persistent ledger, production runtime, mobile/true-device/live proof, UIUE payload contract/consumption, and UIUE merge remain future proof. Preserve original row. |
...
| C061 | CE-008 | P2 | mainline_first | Rewrite | 19.3 | retry/idempotency 规则: 重试不得二次写 state 或吞掉 no-op。 | D13 disposition: `covered_by_D13_c3_runtime_adapter_local_unit` for C3-path per-transition command identity, in-memory retry replay no second write when retry reaches adapter, idempotency conflict fail-closed, fanout identity, and failed-command no fake success in main commit `612e0df`; persistent retry ledger, exact stale retry before C3 stale-state guard, production runtime integration, mobile/true-device/live proof, UIUE payload contract/consumption, and UIUE merge remain future. Preserve original row. |
