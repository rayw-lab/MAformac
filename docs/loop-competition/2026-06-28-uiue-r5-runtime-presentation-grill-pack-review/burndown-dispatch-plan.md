...
| C005 | RPB-05 | P1 | mainline_first | Keep | 21.5 | 写所有权: 触摸/事件走 executor 或 runtime adapter, 不直接写 store。 | D14 disposition: `covered_by_D14_c3_runtime_adapter_residual_local_unit` for C3-path adapter-owned mock write proof plus session-scoped success/failure ledger, readback reconciliation, and settled stale replay no-mutation proof in main commits `5d0cd27`/`66dda25`; D15 adds presentation-safe payload contract but no UIUE consumer. Durable persistence, production runtime, mobile/true-device/live proof, UIUE payload consumption, and UIUE merge remain future proof. Preserve original row. |
...
| C061 | CE-008 | P2 | mainline_first | Rewrite | 19.3 | retry/idempotency 规则: 重试不得二次写 state 或吞掉 no-op。 | D14 disposition: `covered_by_D14_c3_runtime_adapter_residual_local_unit` for C3-path per-transition command identity, session-scoped retry replay/no second write, idempotency conflict fail-closed, failure ledger, readback reconciliation, exact settled stale retry before stale guard, and parent request fingerprint + settled parent plan in main commits `5d0cd27`/`66dda25`; D15 exposes only presentation-safe reconciliation status, not private retry/ledger fields. Durable retry ledger, production runtime integration, mobile/true-device/live proof, UIUE payload consumption, and UIUE merge remain future. Preserve original row. |
