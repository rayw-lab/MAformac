...
phase2_product_coding: G5_COMPLETE
...
1. **G5_COMPLETE**（工程）；下一刀 **G6** — receipt / proof 链路（按作战包；勿抬 proven）。
2. 严格遵守 shared-file 串行 owner；candidate 家族零演示、零合并、零翻 `actionDemoProven`。
3. residual 强制账（不因 G3/G4/G5 推进而消失）：BF-3 candidate 冻结；BF-6 UI-E2E cancelled+未 required；BF-7 C5/full suite；BF-8 人类亲验；BF-10 protected dirty；`actionDemoProven=0/120`。
4. 禁抬 proven；禁演示后三族。
