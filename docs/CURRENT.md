...
phase2_product_coding: G6_IN_PROGRESS
...
- **G6_IN_PROGRESS · 刀1**：`RuntimeTurnReceipt v2` clean-cutover + route-level Assembler + B10 durable 写失败面（run-root `PHASE2-G6-KNIFE1-RECEIPT-V2-CLOSEOUT.md`）。仍 **`PHASE2_CODING_GATED`**；**未抬** proven；后三族禁演示。
...
1. **G6_IN_PROGRESS**：刀1（Receipt v2 + Assembler + B10）已落；下一刀 **G6 刀2** — schema/digest tests + 双 turn linked 硬化（勿抬 proven）。
