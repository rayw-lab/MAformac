...
## 当前阶段（2026-07-02 深夜：tiny-ablation v5 重标 + v6 契约重构 Phase 0-3 进行中）

🔴 **裁决-A 首跑（v5）verdict = `BLOCKED_INVALID_FOR_PARADIGM_VERDICT`**（四根因：监督残缺/探针构成/输入面/基线锚——终版跨 LLM teardown `docs/c5-training-readiness-grill/tiny-ablation-iceberg-teardown-FINAL-2026-07-02.md`）。**v5 不得作为 D-domain/LoRA 范式失败证据、不得据以调 LR/rank/scale/clip/iters**。磊哥六拍（D-027）：重标级联/both 分轴/A+ 契约/base 配对重锚/Phase 0-3 授权（docs-code-test only）/R7 续签至 **2026-07-23**。v6 rerun 需按 FINAL 档 §5 另签 run-auth。

## 前情（2026-07-02 晚，M1 已收口）
