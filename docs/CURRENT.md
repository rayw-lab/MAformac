...
## 当前阶段（2026-07-03 午后：**T1 smoke FAIL / Metal OOM**，训练线暂停待 T1D）

🔴 **D-053 最新真态（supersedes 本文件下方“N4 等 4 键 / N5E 锁定”旧等待态）**：六支合并链已完成，main pin=`b33d8eba152e5326f69bbe85fc356b73419ee9c3`，PR31 final 新验收基线落定；run-auth 后 T1 smoke 已真跑，结果 = **FAIL**：validation `val_loss=3.081` 后、首个 optimizer update 前触发 Metal OOM，`optimizer_update_count=0`、adapter 未保存、watchdog 未触发（非 hang）。正式训练继续 BLOCKED，下一步只能走 T1-OOM 诊断矩阵，不得写 train-ready/V-PASS。

📌 **新指针（D-054 advice landing）**：`docs/c5-training-readiness-grill/t1-oom-premortem-iceberg-advice-2026-07-03.md`（给 commander 的结论口径） / `docs/c5-training-readiness-grill/t1-oom-diagnostic-runbook-2026-07-03.md`（T1D 一变量诊断 runbook） / `docs/c5-training-readiness-grill/token-budget-supervision-ledger-2026-07-03.md`（token 双账：长度 vs 监督面）。当前 token 真态：E-2 长行有效（max 7185/7186 <8192，length violations=0），但 PR31 final 监督面扩大（trainable_tokens `44459 -> 113914`，约 2.56x），这是 T1D 必须显式纳入的显存风险变量。
