...
- **G0 inventory COMPLETE**；**G1 首刀 GREEN**（`abfe715a`）→ **G2 首刀 + 余量 GREEN**（`f7599b5c` / `745f1c5b`）→ **G2-P3 °F 客户入口 GREEN** → **G3 首刀 GREEN**（`4f38ab14` fresh-risk）→ **G3 刀2 进行中**（window CUR quartet 20→70 / range / generic；未抬 proven）。仍 **`PHASE2_CODING_GATED`**：非全面解冻；禁演示 / 抬 proven。
...
1. **G3 刀2（window CUR quartet）合入本树后**：下一刀串行 **row167 compound atomic**（0–3 transitions / rollback；禁与 CUR 同 commit 抢做）。
