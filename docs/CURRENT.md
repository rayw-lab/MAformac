...
2. **G4 刀1 GREEN**（`f5f00395`）→ **刀2 GREEN**（`528eefeb`）→ **刀3 GREEN**（`b11184fd`+`a4924a81`；closeout run-root `PHASE2-G4-KNIFE3-APP-TASK-HANDLE-CLOSEOUT.md`）：Composition 持 `ingressRouteTask`；cancel→`markCurrent`→`cancelPendingSpeech`→新 Task；完成后 defer 清柄；lease→runner；ContentView 无匿名 route Task。下一刀：刀4 竞态场景 + G4 smoke 聚合。
