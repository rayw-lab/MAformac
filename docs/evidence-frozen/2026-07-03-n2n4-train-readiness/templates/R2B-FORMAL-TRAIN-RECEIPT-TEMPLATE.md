# R2B-FORMAL-TRAIN-RECEIPT — template

status: FILL_AFTER_RUN__r2b_formal_train_health_{pass|fail|partial}
...
created_for: R2b formal train receipt, fill within 5 minutes after training completion
claim_boundary: This filled receipt may only claim formal training pipeline health. It must not claim eval PASS, train-ready, C6 acceptance, V-PASS, or product acceptance unless the separate eval/verdict gates also pass.
...
| shorttrain receipt + eval verdict | formal-train prerequisite | formal training must not inherit shorttrain PASS without explicit prerequisite rows below |

## Fill Rules

- Replace every `FILL_AFTER_*` before declaring PASS.
- Formal receipt must bind the exact same or explicitly approved successor basis. Any code/data/config drift must be marked `BASIS_CHANGED_REQUIRES_REVIEW`.
- Do not reuse the shorttrain adapter sha as formal adapter sha.
- Do not treat formal train-health PASS as eval PASS; eval belongs to the formal eval receipt.
- Efficiency columns are mandatory: `supervised_tok_per_sec` and `ignored_trainable_ratio`.

## Prerequisite Gate（正式训练起跑前/收据中必须留痕）

| prerequisite | expected evidence | value | 判 |
|---|---|---|---|
| shorttrain train-health receipt | R2b shorttrain receipt path + sha | FILL_AFTER_SHORTTRAIN__receipt_path_and_sha | FILL_AFTER_RUN |
| shorttrain eval/verdict | R2b eval verdict path + summary | FILL_AFTER_SHORTTRAIN__eval_verdict_path_and_summary | FILL_AFTER_RUN |
| commander go/no-go | explicit formal-train authorization or runbook line | FILL_AFTER_RUN__authorization_ref | FILL_AFTER_RUN |
| LR / packing / PEFT drift | should be unchanged unless commander explicitly approved | FILL_AFTER_RUN__unchanged_or_decision_ref | FILL_AFTER_RUN |
...
| adapter | FILL_AFTER_RUN__formal_adapter_path; sha `FILL_AFTER_RUN__formal_adapter_sha256`; checkpoint `FILL_AFTER_RUN__checkpoint_id` | `shasum -a 256`; checkpoint directory listing | FILL_AFTER_RUN |
| config | FILL_AFTER_RUN__formal_config_path; iters `FILL_AFTER_RUN`; optimizer updates target `FILL_AFTER_RUN`; batch `FILL_AFTER_RUN`; grad_accum `FILL_AFTER_RUN`; token_budget `8192`; grad_checkpoint `true`; rank `16`; lr `FILL_AFTER_RUN` | config file + run metadata | FILL_AFTER_RUN |
| command | FILL_AFTER_RUN__exact_formal_train_command | shell history / runbook / train.log header | FILL_AFTER_RUN |
| run dir | FILL_AFTER_RUN__absolute_formal_run_dir | filesystem + mtime | FILL_AFTER_RUN |
...
| final adapter sha | FILL_AFTER_RUN | `shasum -a 256` | FILL_AFTER_RUN |
| post-train eval handoff ready | FILL_AFTER_RUN__eval_runbook_path_or_command | eval runbook / commander handoff | FILL_AFTER_RUN |
...
Purpose: make data order and replay/protected exposure reconstructable. This section closes W27 T7/P1-3 only as a receipt template requirement; the filled receipt still needs run-local evidence.

### Prelaunch Frozen Data Basis

| field | frozen value | source | fill rule |
|---|---|---|---|
| trainpack samples | `wave2-fix/r2b-trainpack/c5-training-samples.jsonl`; rows `5499`; sha `3b76d49767e650b2cf5d41a3dd6b527932c1e2e2500bb249ac917303380f84e0` | `F044-R2B-DATA-READY-RECEIPT.md`; `formal-eval-manifest.json` | Must match before formal launch. |
...
| bucket | prelaunch row count | source | effective formal exposure count | verdict |
|---|---:|---|---:|---|
...
```text
R2B-REPLAY-SHORTFALL-01 accepted: substrate lacks already_state/unsupported analog replay sources; 102/647 remains inside the 10-20% band. Formal training must carry this ruling forward or explicitly reopen it before launch.
```
...
- [ ] Final formal adapter exists and sha is recorded.
- [ ] Required checkpoints exist or missing checkpoint is explained.
- [ ] Source snapshot exists in formal run dir.
...
- [ ] Formal train used the authorized config and did not silently add LR sweep, packing, QLoRA, DoRA, or mount changes.
- [ ] Formal adapter is handed to eval with adapter sha and bundle sha; no eval claim is embedded in this train-health receipt.
...
|---|---|---|---|---|
| train-health | `local/metrics_jsonl_firsthand_scan` | Formal run completed/failed/partial with health evidence. | Not F044 behavior PASS. | This receipt. |
...
This receipt is DONE only when all prerequisite, basis, health, efficiency, and stopline rows are filled from run-local evidence. If any one is missing, final status must remain `partial`.
