...
5. **🔴 竖屏三屏分层空间挤压(磊哥 pre-mortem,记 AD-11 防 Phase 5 返工)**:三屏分层(orb 顶/对话流中/车控卡片下)在 iPhone 竖屏 ~874pt 空间紧张——10 族 2 列×5 行 ~390pt + orb~220 + 对话流~240 = 850pt 顶满,10 族被压矮或要滚 → 撞「全景常驻一眼看全」。**修法(Phase 5)= 动态非固定三等分**:idle 态 orb 收小球(车控主体 ~70% 看全景)/ think-speak 态 orb 放大顶+对话流浮现+车控下沉底部 ~45%(ScrollView 内滚+激活族自动滚入视野高亮)。横屏(Mac 5列/iPad 4列)无挤压。**元规则:多区域垂直堆叠布局,设计时先算各区 pt 高度和 vs 屏高,超屏=动态弹性伸缩非固定均分;4a 车控层本身正确,三屏分配是 Phase 5 实装点(agree-before-build,Phase 5 grill 细化比例)。** ⚠️ **此条「动态分配」被 6-lens 调研修正为「固定全景 idle(守 spatial memory)+ 活跃族原地放大 hero 不物理重排 + ScrollViewReader 自动滚」(承接 D4,优于物理置顶零重排破坏),见 design AD-12。**

6. **🔴 force-state 验收脚手架 ≠ 真实台本(体验审计 catch,claim-vs-reality 第10变体扩)**:14 张 force-state 截图为「满屏丰富视觉」5-gate 脚手架(`DebugGallery.forcedScenarioCells` 写死 5 族活跃只 ac 卡切态),**不代表任何真实台本时刻**——真实冷启动(`defaultCells` 全 normal 静默)从未被截图验证,我却在 proof README 用 force-state 图背书「用户真实看到」。subagent CC 用户演绎体验视角 catch:① 占位卡「未激活」客户读成「demo 没做完」撞惊艳门(→改「待命」就绪态)② 真实冷启动可能满屏灰(→补 `ios-coldstart-real.png` 无 force-state 验证非 broken)③ scope 淡角标 9pt 投屏看不清落空裂缝⑤初衷(→提对比 caption semibold,淡≠隐形)。→ **元规则:验收脚手架(force-state/mock 注入)截图证「机制成立」≠ 证「真实台本体验」;必补【真实运行态(无注入)】截图验证开场/单族激活/低排位滚动;视觉验收派 subagent CC【用户演绎体验视角】(方案经理 5min 台本+客户旁观者+corner case),catch 代码/高清图看不出、只现场视角暴露的呈现语义缺口(占位文案/开场灰/角标投屏/态色误读)。** 与 aesthetic-first 验收纪律(还原用户实际查看环境)同源,补「脚手架图≠真实台本」维度。

7. **🔴 goal/dispatch 切片机械执行 = 丢派单全貌 + 丢 grill 承接(沉淀 rule `goal-dispatch-trace-to-source`)**:`/goal「完成 Phase 4a」` 我机械执行 4a Task,没回溯①派单计划是 Phase 4 全体(4a/4b/4c)②D1-D6 grill 已深决交互框架(全景常驻/触发聚焦/iPhone三zone/活跃置顶/sequencer)→ 自拍竖屏布局(D4 早决)+局部最优(4a 接不上 4b/4c)。磊哥连环纠「这个你就做决策了/grill 这么久没聊过/派单是 phase4 全体」。→ **goal/dispatch=聚焦执行切片(派生),派单全貌+grill SSOT=一手;执行切片前必回溯两源(读计划全貌+grep grill 决策),别机械执行切片当完整 spec。** 全局 rule 已沉淀。

8. **🔴 cross-vendor(codex/OpenAI) 抓到 3 轮 Claude 同家族漏的 bundle 化 gap = dev/模拟器≠打包真机 frame 盲点(cross-vendor-final-audit 实证)**:Phase 4a 审计 3 轮全 Claude(2 adversarial + 1 体验)+ 我主线程亲核,**全聚焦 Presentation 层逻辑 + 跑 swift test(用 #filePath 源路径,dev 态可用)**,一致漏了 `StateCellPresentationCatalog.loadStateCellsYAML` 的 **bundle 化缺口**:打包 .app Resources phase 空 + SPM exclude contracts → 真无 state-cells.yaml → 退 `#filePath` host 源路径 → **Mac/模拟器工作(host 解析)但真 iPhone standalone(该路径不存在)catalog 空 → default_scope 退化**。codex 从 `Package.swift exclude` + `pbxproj Resources files=()` **两线交叉**坐实是真 P1。→ **元规则:① cross-vendor 终审必派(≥1 真 cross-vendor 非 Claude 同家族);同家族 N 轮审共享同一隐式 frame(「dev/模拟器态=真机发布态」),抓不到跨 frame 盲点(claim-vs-reality §16/§31 实证)② 任何「从 bundle/文件加载」的 catalog,必验【打包产物(.app/jar)真有该文件】不是只验【dev/test 态(#filePath/源路径)能读】——`#filePath` fallback 是 dev 糖,真机不存在;bundle 化缺口在模拟器(host FS 共享)隐形,真机才炸。** 与 active-probe-over-config-check(状态≠行为) + claim-vs-reality 第10变体(派生表征当一手)同源,补「dev 态≠打包真机态」维度。修法见 design AD-12 §十(bundle 化 / codegen typed catalog / bundle 存在性测试)。

9. **🔴 UIUE 是「语义呈现层(Presentation Contract)」非「UI 改版」——下一刀补语义层非动画(gptpro 产品架构意见,磊哥 4b 前定盘)**:gptpro 高层意见点破 4a 进入「语义呈现层」开发(把 C2 端态协议/scope 语义/族级信息架构/演示叙事/视觉状态机压进秒懂界面),不能再按普通 UI 组件心智写。**核心论点:scope 聚合不是 UI 文案是 per-base semantic aggregation problem;`default:.badge` 把「真 badge」与「忘配置被吞」两语义混一起=4b Gauge 追查灾难;UIUE 正确抽象是 B1-B5 Presentation Derivation 三层(FamilyCardIDMapper/FamilyPrimaryCellMapper/UIValueTypeMapper/ScopeAggregationResolver/DominantVisualState),B4 还太弱,下一刀最该补 ScopeAggregationResolver 非动画**。→ **元规则:① 进入语义呈现层后,优先级 = 语义安全带(聚合 resolver/契约闭合/穷尽 enforce)> 视觉动画;② `default:` case 禁同时表「合法兜底」与「漏配吞错」两语义——漏配必 assertionFailure + contract-driven 闭合测试(遍历 contract 全 id 必显式映射或 allowlist 或 deferred 标记),别让 default 成「沉默垃圾桶」;③ 消费侧第二份 SSOT(FamilyPrimaryCellMapper 等)必契约存在性测试约束(防 A2 改 contract 后静默漂移);④ deferred 必 phase matrix 追踪(非注释散落),防 reviewer 误判 deferred=缺失 / 真缺失混入 deferred。** 4b 调顺序:先语义层 hardening 再 value.type 控件。

10. **🔴 §28 一手源:写新类型/解析前必 grep 现有 codebase 同名/同职责(没核就重复造,4b ValueRangeMapper 实证 + 认知到≠行为改)**:4b 写 `ValueRangeMapper` 时新建 `struct ExecutionRange{min,max,step}` + 给 `StateCellPresentationCatalog` 手写 `execution_range` 解析,**没先 grep**——A2 `Core/Contracts/ContractLookups.swift` 早有 `public struct ExecutionRange`(同字段)+ `StateCellContractLookup.cell(id:)→StateCellDefinition`(完整解析 state-cells.yaml 全字段:type/scope/defaultScope/**executionRange**/readback/gearMap)。编译 `'ExecutionRange' is ambiguous` 才暴露。→ 回退:删重复定义用 A2 类型 + `extension ExecutionRange{closed,stepCount}` + catalog **委托** `lookup.cell(id:).executionRange`(execution_range 单一解析)。🔴 **更深 catch:`StateCellPresentationCatalog`(前任 4a 建)本身就与 A2 `StateCellContractLookup` 重复**(title/scope/defaultScope 双解析同一 yaml)= 第二份 SSOT(gptpro 第6点 + derivation 铁律2 活实例),4a 遗留记 `design AD-13 phase matrix` harden(后续 catalog→A2 lookup 委托,上抛磊哥重构范围)。→ **元规则:写任何新 struct/enum/解析逻辑前先 `grep -rn 'struct X\|同职责' Core/`——同名/同职责已存在就复用(§28),别造第二份(尤其契约解析:同一 yaml 多处手写=漂移源)。🔴 认知到≠行为改:我【刚沉淀 derivation 铁律2「消第二份 SSOT」转头就造第三份】(claim-vs-reality 第8坑变体)——扳机在【写新类型/解析的那一刻】grep,不是【沉淀完规则】就免疫;effort/警觉不替代动作点 grep。**

## K. UIUE 视觉/交互 grill marathon + runtime bridge + diorama 调研教训(2026-06-25,SD18-25 + A-1 bridge + context capsule)

1. **🔴 技术方案推荐前必先 grep 项目内已拍约束(claim-vs-reality 在「技术选型」维度,深调研才 catch)**:推 context capsule diorama 走 C(含玻璃折射 `layerEffect` shader),外部 github-first 调研(Vortex/Inferno/折射 shader 教程)做了,但**没先 grep 项目内已有 shader 决策**——U30/U31 早定「**layerEffect 最贵,与 mlx 抢 GPU -50%,shader 仅氛围层 + 有效性 spike 实证不拍**」。继续深调研 + blueprint-teardown(Vortex=Canvas 非 layerEffect 便宜)才撞到,改推 **C-lite**(砍重折射 shader,native glass + Vortex 粒子 + image offset)。→ **元规则:技术方案推荐不只外部 github-first,先 grep 项目内已拍技术约束/决策(U 系列/grill SSOT),别在项目已否决/已约束的方向上推荐;claim-vs-reality 扩到「技术选型前先核项目内一手约束」。**

2. **🔴 外部参考物(AI 生成图/调研产物)非权威,项目 grill SSOT 才是(磊哥 catch「不要以 gpt 为准」)**:gptpro 生的 5 张 diorama photoreal 图惊艳,我一度过度尊崇(措辞「惊艳/photoreal 拉满」);磊哥纠「**不要以 gpt 为准,我们要有自己的布局,他毕竟只是锚点图,有的图设置刷新弄进去了**」。gpt D4 雨夜图把刷新/设置弄进 capsule 内 = 违 SD24「图标右上角 standalone 在 capsule 外」,不采纳。→ **元规则:外部派生参考(AI 图/调研产物)只借【视觉/技术灵感】,布局/契约/spec 以项目 grill SSOT(SD/AD/RPB)为准;参考物有错(图标位置/布局)别让它覆盖已定 spec。derivation-layer + claim-vs-reality(派生表征非一手权威)在「外部参考」维度。**

3. **🔴 grill「已拍」≠「已落地」,sweep 揭示主体是「拍了没落」(completion-claim-triage 落地版)**:234 grill 决策,磊哥问「都确认完毕了吗」;做 landing matrix sweep(grep `openspec/specs`+`changes`+`Core`/`App` code)= 已落(契约 C1-C6 + 4a/4b/4c code)/ **仅 grill 待落(本 session SD18-25 + V + RPB 53 + default-scope,主体)** / DEFERRED。→ **元规则:断言 grill「完成/确认」前先分诊【决策态 vs 执行态】(completion-claim-triage);多轮 grill 后做 landing matrix(每项 [已落 spec/code / 仅 grill 待落 / DEFERRED / superseded]),别把「拍了」当「落了」;连续暴露的 gap(runtime 派生层/层级滚动/already_state)证明 234 本身有洞,grill 是否穷尽也存疑。**

4. **route/技术形态不文档拍死,spike 实证(U31「spike 实证项不拍」+ 实跑非推测)**:capsule 动画 photoreal 3D vs 2D 卡通 vs 2.5D hybrid,GPU 约束(U30)+ 像不像图 + 真机帧率都是推测;不在文档拍死,留 A-2 真机 spike(A 视频 loop vs C-lite,量帧率+观感)一手定。→ **元规则:高不确定技术形态(shader 有效性/性能/观感/真机表现)= spike 实证项不文档拍(项目 U31 已定),claim-vs-reality「机械操作/性能实跑非推测」。**

5. **审计员 finding 全属实仍逐条核一手(claim-vs-reality §28 正面应用,不盲信「PASS」)**:外部审计 A-1 给 verdict `PASS_WITH_P1_FIXES` + 3 finding(CURRENT stale / tasks isComplete 分叉 / ScopeOrigin 无 missing),我没盲信,逐条核一手(读 `CURRENT.md`/`openspec status`/`ScopeResolution.swift`)全属实才修,且**纠正审计措辞**(审计说「store 有 ScopeOrigin」,实际 `ScopeOrigin` 在 `ScopeResolution.swift` 非 store;store 是 `DemoVehicleValueSource`)。→ **元规则:收外部审计(含说「属实/PASS」的),仍核一手坐实再动,审计也会措辞不精确;cross-vendor 审计是发现器不是权威终判。**

6. **🟢 cite-verify hook 机械门边界 + commit 卫生(本 session 实操)**:① cite-verify hook 反复把 `RPB-30/51`/`534/2086` 等【ID 引用/作废历史值】误判为「无源数字」(正则把 `N/M` 当分数)→ de-slash(`RPB-30 与 51`)或去掉作废值即解;hook 治 **mechanical**(value-in-source/格式),治不了 correctness,误报照 de-slash 不纠结。② commit:**可再生大二进制(42M AI 生成图)gitignore 本地 + prompt 源(.md)入仓**(仓库卫生:再生物不入仓);machine-specific symlink(`Tools/agent-platform-plugin-refs/` → `~/.codex/plugins`)必 gitignore。
