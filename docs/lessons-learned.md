...
codex/worker 的脑暴/调研/计划=单 frame 草稿非结论。收稿即交叉派异 worker 对抗 grill（cite-verify+steelman+缺维扫描），commander 只信合成。三层核验体系：D-089 亲核（我信了什么）/本条互核（它漏了什么）/cross-vendor 终审（体系漏了什么）。实证：R3-AMMO-1 对抗深挖翻掉三轮误诊断（11/64 eval case mount-invalid=量尺坏非模型差，唯 cite 到 probe JSON 一手才见）。**FP 救回子条款（2026-07-08 D-122 holdout repair）**：checker 命中≠真撞（semantic dup 受 tool args/slot/safety expected=[]/双意图结构影响），FP 救回须 ≥2 票同向+分歧从严剔+合成表逐行留理由（锚 `holdout-neardup-fp-synthesis.md:6,13-35`）。
...
## M.48 near-dup 剔除/补行后 judge 必全量重判，不只判新增行（2026-07-08 D-122/D-127 holdout repair）
holdout 剔/救/补后，曝光面是全局性质（旧行在新排布/checker 下可能暴露新撞车），非 append-only 局部性质。每轮 J judge 跑完整 file+exposure，全量写 rows/buckets/sha；「新增行已绿」不能替代整体绿。实证：J2/J3 两次全量重判各翻出增量对句（032/044 救回即由此而来）。锚 `decisions.md:1145-1147` + `daywork/reports/J4-REPORT.md`。

## M.49 补行生成 SPEC 必逐字段 inline，禁「沿用前批」（2026-07-08 D-122 Opus 补行）
生成位只消费当轮 prompt，「沿用前批」把 schema authority 变成记忆和猜测（旧字段名/旧口径回流，如 `bucket` vs `holdout_family`）。SPEC 逐字段 inline+生成 receipt 自检点名真实字段+judge 逐字段核 schema。锚 `decisions.md:1107` + `daywork/reports/J3-REPORT.md`。

## M.50 多 worktree 集成预演先审 merge-base+dirty state+moving-HEAD（2026-07-08 D1a 集成）
多 worktree 并行时 branch base/目标分支 ahead/实时 HEAD 都在漂。集成预演先写 merge-base、dirty state、moving-HEAD caveat、收编顺序建议，再谈 merge。实证：T1 曾基于 main 非目标 opt/streamline；IN1/IN2 两轮只读预演后五片零冲突收编。锚 `daywork/integration-rehearsal.md:20-24` + `integration-rehearsal-v2.md:10-17`。

## M.51 local runtime proof 是可见副作用，必须 idle-window 化（2026-07-08 T6 抢跑弹窗）
macOS 前台 app/screencapture 抢用户注意力，不能当无副作用后台任务。用户用机立即停前台动作；已产物保留，剩余流程封装 run-when-idle 脚本，状态写 PARTIAL_PENDING_IDLE_WINDOW。姊妹坑：工作屏截图可截到下层窗口（飞书=隐私，memory `macos-gui-screenshot-privacy`；T6a 三张污染图勿外发）。锚 `daywork/d1a-harness/t6-receipt.md:19-23`。

## M.52 receipt 数字默认 stale，对抗审必记 live HEAD/status/test output（2026-07-08 T7 对抗审）
并行 worktree 在审计期间继续移动，旧 receipt 的 HEAD/test count 自然过期。对抗审记录 live 三件并把 receipt drift 当发现写出，不沿用旧数字。实证：redraw3-summary 报 P2=3 实况 fix 后 P2=0（dated report < live 门）。锚 `daywork/t7-adversarial-audit.md:18-19`。

## M.53 补行配额按桶算余量，不按总数标量替换（2026-07-08 D-122 holdout repair）
coverage 门是 per-bucket 约束而 near-dup 剔除桶分布不均，「剔几补几」会让弱桶（particle/topic 低频桶）继续不达标。先算 post-exclude 每桶缺口再加 reserve，配额写成 family table。锚 `daywork/holdout-neardup-fp-synthesis.md:31-35` + `decisions.md:1125`。

## M.54 args 修复只碰 expected_tool_calls/signature，禁偷改 utterance/register/bucket（2026-07-08 HF1）
judge FAIL 的原因是 expected schema/slot 非样本本身；顺手改 utterance/register/bucket「变绿」会污染 holdout lineage（身份字段=样本身份）。只改 expected 面，复核确认 utterance/桶/register 零动。实证：HF1 三行修正（value=2/20/暖香槟金）后 J4 61/61。锚 `daywork/reports/J3-REPORT.md` + D-127。

## M.55 near-dup checker 是预检门不是语义裁决官（2026-07-08 D-122）
语序层 checker 实战有牙（四连证）但抓不到低 lexical overlap/安全上下文反向语义/expected=[] 结构差异。checker rc66 只触发人工/异源 judge，最终保留/剔除以 full judge+synthesis receipt 为准。锚 `decisions.md:1147` + `daywork/holdout-plan-redteam.md:40`。
...
## M.78 authority 的 docs-only commit 也会打红 canonical closure：改 decisions 必须进入显式事务（2026-07-11 ma15）
D-149 commit `d6796e83` 只改`decisions.md`与handoff、无产品代码，却改变authority SHA，使closure checker 5项共同报`E_DECISION_STATE`；这与D-148 merge-context rebind同族，但新增“纯docs提交也触发”的变体。机理是registry把decisions digest与basis作为可验证authority状态，文件类别不影响哈希失配。修法：authority变更必须进入显式cascade transaction；单commit时pre-commit要求authority+registry/receipt/roadmap同笔，A=authority/B=reanchor两commit时允许A暂红但须标transaction-pending、A/B间禁push；pre-push/transaction-closeout必须亲跑`make verify-closure-work-packages`+`make verify-ci`，读取直接rc均0且subject=最终HEAD才放行，负例“只推A”必须被拒。锚 `runs/2026-07-11-ma15/reports/FIX-REDGATE-REANCHOR-by-w1.md:17-35` + `DRAFT-D150-g3-ratified-v2-by-w4.md:85-111`（后者为制度模板草稿，非authority）。

## M.79 legacy-token 预算红应消除新债，不应用加 budget 把债合法化（2026-07-11 ma15）
D-149新增一处旧动作证明术语，使allowlist计数从预算26增至27，完整`verify-ci`报`E_LEGACY_ALLOWLIST_BUDGET_EXCEEDED`；w1没有调高budget，而把原句精化为canonical `actionDemoProven=0/120`并紧邻追加ERRATA，修后count恢复26、allowlist文件零改、双门fresh绿。机理是budget是遗留债上限和防回潮门，不是正常扩容配额；抬预算会把新增旧语义永久制度化并掩盖迁移倒退。修法：门红默认走“定位唯一新增token → canonical术语替换 → 保留可追溯ERRATA → 证明budget/config未改”；只有独立authority明确批准新的遗留兼容面时才可评审budget变更。锚 `runs/2026-07-11-ma15/reports/FIX-VERIFYCI-LEGACY-TOKEN-by-w1.md:13-22,31-43`。

## M.80 spec sync 与机械全绿不证明行为可达：收编门必须穿过 production-shaped chain（2026-07-11 ma15）
G1-T1旧双P0修后，active spec已允许bounded-reviewed多意图、OpenSpec strict valid，`swift test`与`make verify-ci`均`813/6/0`；fresh复审仍发现P0-03：spec要求“一项可执行+一项未挂载”逐项accepted/refused，但W5d production backend在partial runner前遇任一unmounted call即整批pre-C3 throw，规范场景不可达。机理是parser/cardinality测试、手工frames下游测试与production ingress链彼此断开，机械门只证明各局部既有断言，不证明端到端语义组合。修法：每个load-bearing SHALL除spec validation外，至少绑定一条production-shaped fixture，覆盖真实envelope→backend policy→router→runner→readback/no-write/finiteReason；若生产策略选择整批fail-closed，则必须同步修订active spec，不能让冲突两侧各自全绿。锚 `runs/2026-07-11-ma15/reports/FRESH-REAUDIT-G1-CODE-T1-by-w5.md:14-16,29-41,61-70`。

## M.81 呈拍命题不能只过文本 cite-verify：可行性要有 build spike，拍板对象要绑 exact set+SHA（2026-07-11 ma15）
A4 one-pager与两道配对审都看见`ToolContractCompiler.init(seeds:)→C5SemanticSeed`反向边，却仍把“Xcode-only排除7文件”判断为可独立落刀；实跑exceptions虽令双平台compile-input 7路径全0，Mac/iOS仍同因保留Core签名缺被排除类型而rc65，直到A4-0b先搬dev extension才绿。同期“A5同意”只有自然语言键，exact汇编后才暴露它可能指`G5-016..020`五个A5 child rows，也可能被误读为已拍过的`G5-001/012/021/026/028b`五个escalate items。机理是文本锚能证明“方案写了什么”，不能证明保留依赖图可编译，也不能唯一标识人审对象。修法：呈拍前对load-bearing技术可行性跑最小production/build spot-check（target split至少retained→excluded signature scan+双平台compile spike）；拍板语句必须引用exact ID清单文件+sha256，receipt回读同digest，集合或SHA漂移即停拍重呈。锚 `reports/A4-ONEPAGER-v2.md:14-18,28-42` + `XAUDIT-A4-TARGET-SPLIT-by-w2.md:70,86-88,171-187` + `XAUDIT-A4-ONEPAGER-by-w4.md:16-33,63-65` + `A4-0-EXECUTION-RECEIPT-by-w4.md:14-55`；A5锚=`A5-EXACT-ITEMS-AND-PLAN-by-w5.md:14-28,53-88,152-169` + `KEYS-RECEIPT-ma15-leige-batch2.md:17-20`。

## M.82 从未绿过的门第一次真跑会一次性暴露全部隐藏层：授权解除不是 proof（2026-07-11 ma15）
v5b mandatory ABI 在授权前两次都停在test-method之前；授权修好后才首次真进入方法，随即连续暴露三层：formal外层五键没有传播到UI-test runner、sandbox内`/usr/bin/git rev-parse HEAD`失败却未检查termination status、空SHA进入app后又被`try?`静默吞掉，最终只剩12秒receipt timeout表象。历史扫描确认exact test从未有`TEST SUCCEEDED`，故“授权 blocker 已清”不能推成“ABI曾绿/只剩偶发环境问题”。这类`NEVER_GREEN`链的机理是前置blocker长期遮蔽后置缺陷，第一次真跑不是验证旧假设，而是第一次获得整条链的可观察性。修法：所有从未到达目标方法/阶段的门显式标`NEVER_GREEN`；解除一层后仍按`entry→carrier/env→sandbox/runtime→configuration→writer/readback`逐层留证，缺键/非法值在launch前named fail-closed，禁fallback与静默`try?`；只有exact正式门+完整receipt链才升级proof。锚 `runs/2026-07-11-ma15/reports/V5B-TEST-ICEBERG-by-w2.md:9-17,30-46,56-64` + `V5B-HARNESS-FIX-by-w2.md:14-27,40-55`；后者的proof cap见`:9-10,55,72-74`，仅证明local/build修复就绪，不是operator ABI proof。

## M.83 集合扩张会击穿“单实例时代写死清单”：派单必须 grep 全仓具名 roster 并同步 consumer（2026-07-11 ma15）
今日commander定名的同族三连必须保真：closure probe只拷W1；`CHECKER_PATHS`漏A4；Makefile用`-m unittest scripts/test_x.py`斜杠模块调用，直到主线presence sandbox才暴露。G1-T1新增source-free tests/checker却未进`verify-ci`是fresh复审发现的相邻同族旁例，不属于上述三连。共同机理是fixture copy、checker roster、Make target调用形态等“单实例时代第二SSOT”不会随守护集合扩张自动生长，且常在集合真扩张或主线meta-gate首次执行时才爆。修法不在本条复制第二套制度：凡新增done包/receipt/checker/test/module，派单模板硬扣“grep全仓具名清单并逐处判同步”；可派生则从registry/契约生成期望集，不能派生则加“清单==一手集合”断言，merge后主线全门再宣绿。事故锚 `reports/ICEBERG-TEARDOWN-hardcoded-roster-family.md:10,22-31,49-62`；G1-T1旁例锚 `FRESH-REAUDIT-G1-CODE-T1-by-w5.md:48-55`；跨项目执行规则见 `~/.claude/rules/roster-sync-on-set-expansion.md:3-16`。

## M.84 commander 亲笔件也必须先独立审再实证：作者权限不能替代证据等级（2026-07-11 ma15）
commander wave-2预裁把11题写成“一句全⭐”，独立复审仍抓出5个P1：多数题并非已决忠实派生、M16-010漏`proof_runtime` recipe、W8“v2未落”在成文前已stale、M16-002越出本题授权边界、拍后级联偷越另键stopline。问题不在作者资历，而在亲笔稿同时承担综合、压缩和裁决时更易把`producer default`压成`ratified carry`、把时间敏感真态冻结成旧快照、把一句话跨过多个authority gate。修法：commander亲笔件进入呈拍/派单/正典前同样过`independent documentary audit→live file/hash/status refresh→必要的build/runtime spot-check→修订后readback`；审计必须逐命题分`LOCKED_CARRY/NEW_BALLOT/MIXED`，作者不得自审自证。锚 `runs/2026-07-11-ma15/reports/XAUDIT-COMMANDER-PRERULING-by-w4.md:4-14,18-56,64-70`。

## M.85 异构蜂群的哨兵有两层 ABI：后台唤醒方式与 busy grammar 都必须显式校准（2026-07-11 ma15）
commander在ma15同一session四次沿用shell机械手癖，以`...swarm-sentinel.sh &`重挂哨兵；进程即使存活，普通`&`也不给Claude工具层建立可回调的后台task，无法靠完成通知唤醒指挥官，改用Bash tool的`run_in_background=true`后才产生`Background command ... completed`通知。同期idle-scan又暴露vendor差异：Codex忙态显示`Working (`/`esc to interrupt`，Grok忙态是`[stop]` spinner，原先只grep Codex形态会把Grok误判idle。两者同属“把交互宿主ABI当普通shell/统一终端文本”的类别错。修法：哨兵挂法进入commander checklist，固定为平台原生background handle+task notification+output receipt/readback，禁裸`&`代替唤醒契约；阵容变更时维护`vendor/model → busy/idle markers`表，逐pane capture校准并以golden fixture跑正负例，未知marker fail-closed为`UNKNOWN`而非idle。四次裸`&`一手锚 `~/.claude/projects/-Users-wanglei-workspace-MAformac/f746e4d2-8400-4e6b-bee6-6c8d5a476d24.jsonl:981,1609,2391,2571`；原生background与完成通知锚`:1997,2008-2023,2406,2418-2424`；当前sentinel的异构busy regex锚 `runs/2026-07-11-ma15/scripts/swarm-sentinel.sh:20-31`；既有recognition边界见 `~/.claude/rules/swarm-commander-recognition.md:15,19,25,27-30`。

## M.86 grok/异构 TUI 的 REPORT=advisory ping：先报后写/DONE 后重写三连实证（2026-07-12 ma17）
grok round15 连发三份 REPORT：①空 SHA（文件未落盘即发报）②SHA=`8fc99350`（commander 已核）③SHA=`ace591f0` + 计数修正 B1=13/AUTO=9（DONE 后重写文件）。三连报在 ma16 W3 post-DONE rewrite 之后再次暴露异构 TUI 的写盘/REPORT 时序问题。机理是 grok 的 TUI 层在文件写入完成前即可发出 REPORT marker，且 DONE 后如需修正会原地重写文件而非另出 vN，导致收稿方无法依赖 marker 判断磁盘状态。修法：grok 的 REPORT 一律视为 advisory ping，收稿只认 file-first 三门（文件存在→SHA 可算→内容可读）；DONE 后要修必须另出 vN 文件，禁止原地重写；派单模板加协议硬段「写完→自 hash→单次 REPORT」。锚 `reports/INCIDENT-LOG-ma17.md:1-3` + `GRILL-PREP-INDEX-ma17-v1-by-w1.md`→`v2`→`v2.1`→`v2.2` 四代 SHA 链（`ace591f0`→`3e22b25f`→`e5ef61aa`→`a14a0e27`）。

## M.87 呈拍页协议：exact 句逐字+packet 全哈希 join+批量回复预定义 anchoring+非键区物理隔离去星+文件不可自嵌自身 sha（2026-07-12 ma17）
ma17 多轮呈拍暴露五项呈拍页协议缺陷。①**exact 句逐字**：键5 exact 句首次实现逐字从 packet 拷入呈拍页，消除「自然语言摘要→磊哥读后理解偏差」的中间层。②**packet 全哈希 join**：每键 subject 必须引用 exact 文件 SHA-256，receipt 回读同 digest。③**批量回复预定义 anchoring**：「全部按⭐」「批二全部按⭐」类回复须绑定页 SHA 才有效，防止自然语言「全部同意」在页漂移后 scope 歧义。④**非键区物理隔离去星**：S8 状态区标 `NON-SELECTABLE` 且无 ⭐，防止 grok 异源审抓到的「软级联 frame」——三键全 ⭐+编号序在视觉上等价于推荐一次做完，与「相互独立」规则对冲。⑤**文件不可自嵌自身 sha**：v2 草稿曾内嵌假自身 SHA（文件不可能包含自己的哈希），v3 改为 `page_sha_note` 模式——本页 full SHA 由呈拍消息携带并作为 receipt join 元素。机理是呈拍页同时承担展示、锚定和授权引用三重职责，任何一层协议缺口都会导致收稿/审计歧义。修法：呈拍页模板固定五要素——exact 句 blockquote、packet SHA 表、批量回复 anchoring 规则、非键区物理去星、`page_sha_note` 声明。锚 `PRESENTATION-PAGE-3KEYS-S8STATUS-v1-by-commander.md`→`v2` 链 + `PRESENTATION-PAGE-BATCH2-3KEYS-by-commander.md` + `XAUDIT-GROK-CROSSVENDOR-PAGE-AND-DRAFTS-by-w1.md:42-49`（P1-PAGE-001 软级联 frame）+ `XAUDIT-APPLY-V2-DELTA-GROK-by-w1.md:20-27`（假自身 SHA）。

## M.88 键执行波纪律：mainline 写串行裁决+gate-type task 的 partial_apply_by_design 处置（2026-07-12 ma17）
四键批（键4/5/6/7）执行暴露两条纪律。①**mainline 写串行裁决**：commander 裁决执行序为 ①键4 APPLY（w2，mainline 串行逐包）②键6 编码（w4，隔离树 `c1/g5020-wiring-20260712`）③键5 CI recovery（grok，`dd65a6d3` 支 out-of-band）④键7 级联（w2，APPLY 完+终票后）。一席独占 mainline、编码进隔离树、异支各归各树——防止多 worker 同时写 mainline 造成 basis 漂移和 commit 冲突。②**gate-type task 的 partial_apply_by_design**：W7 apply 后 tasks 19/20 complete，task8（`Before any production consumer coding, obtain RISK-ACK-W7`）诚实未勾。机理是 KEY-APPLY 不授权 coding，故 coding 前置门不能在此键内预签；若强行勾选会制造「coding 门已过」的假证据。修法：apply receipt 显式标 `partial_by_design` + 剩余 task 的 `CODING_GATE_PENDING_RISK_ACK_AT_CODING_KEY` 处置 + 指向 future key 的 exact 材料路径。锚 `KEYS-RECEIPT-BATCH2-PLUS-K7-by-commander.md:12-13`（执行序段）+ `APPLY-W7-RECEIPT-by-w2.md:3-8,19-27`（partial_by_design + task8 处置）。

## M.89 门必须对 live CLI 真输出验绿：jq 期望 vs live .summary.direct、无 --json 则 text_assert 如实标注、空占位 flows=假门必删或 no_gate_by_design（2026-07-12 ma17）
packet v5→v6 审计暴露三类门验收假绿。①**jq 期望 vs live 字段路径**：packet v5 的 gate 期望用 `jq .direct` 但 live CLI 输出字段为 `.summary.direct`，路径不匹配导致 gate 实际未验。②**无 `--json` 的 CLI 输出**：`detect` 等命令不支持 `--json` 时，text_assert 必须如实标注为 `text_match` 而非冒充结构化验证，否则审计无法区分「结构化通过」和「文本碰巧包含关键字」。③**空占位 flows**：占位 `flows=[]` 或 `flows=["TBD"]` 的 gate 既无正例也无负例，通过等于没验。机理是 gate 验收的断言层与 CLI 实际输出层之间存在 schema 漂移，且空占位 gate 在机械检查中永远绿。修法：每个 gate 的期望字段路径必须对 live CLI `--help`/`--json` 输出做一次 exact 匹配；不支持 `--json` 的命令在 receipt 中标注 `proof_class=text_match`；空 `flows`/`checks`/`gates` 数组要么填充真实条目，要么标 `no_gate_by_design` 并在计数中排除。锚 `XAUDIT-PACKET-V5-FINAL-by-w3.md`（jq 路径 mismatch）+ `XAUDIT-PACKET-V6-FINAL-by-w3.md`（live proof receipts + text_assert 纪律）。

## M.90 计划/载力账声称入正典前必须 live 复核：全称判断、异源审并集、祖先绿推断都不能直接升格（2026-07-12 ma18）
ma17 save-pending 给了两条计划层复核纪律：其一，「remote verify 从未真跑成功」这类全称负面判断在搬入 commander 计划前必须 live 复核，源账记录 live gh api 曾给出 602267f1@2026-07-07 旧门面真绿，正确口径应收窄为「门链扩张后未再绿」；其二，双厂商异源审的 finding 集可互补，grok 与 hermes 抓到的缺陷面不相交时，并集才是高价值审计输入，但该结论入正典前仍保留“源记录/待复算”边界。ma18 又用 dd65 证据链坐实更硬版本：A=`dd65a6d3` 是 B=`42408b7e` 的祖先，但 A 自身 detached 读数为 closure `BLOCKED rc=70 errors=['E_STALE_BASIS']`、`make verify-ci rc=2`、pytest `43 passed / 10 failed`；rc0 实为 B reanchor 后读数，祖先/后代关系不能替代 commit 自身 receipt。修法：任何「从未/总是/已绿/已验证」类声称进入计划、KEY、merge 或载力账前，必须绑定自身 live CLI/API/git 或 detached receipt；worker prose、旧 DRAFT、祖先绿、后代绿只能作线索，未复核时显式写 `claim candidate` / `stale candidate` / `inference`。锚 `runs/2026-07-12-ma17/reports/LEARN-EVAL-ma17-by-commander.md:10-12` + `runs/2026-07-12-ma18/reports/DRAFT-lessons-M90-by-w2.md:10-24,28-31` + `runs/2026-07-12-ma18/receipts/AMENDMENT-1-dd65-evidence-recast.md:4-10,12-19` + `runs/2026-07-12-ma18/receipts/hermes-w3-dd65a6d3-detached-verify/RECEIPT-by-hermes-w3.md:14-24,37-40,55-60`。

## M.91 push/promote 放行必须把执行终态、键页 ref、live remote 与观察哨兵分层 join（2026-07-12 ma18，ULTIMATE saga）
ma18 ULTIMATE 首审在本地内容面和五门原始绿证之外抓到两个 P0：一是 `FINAL-RECEIPT.md` 把原始 continuation 的 `BLOCKED rc=152` 事后写成 `DONE/FINAL PASS`，终态脱离脚本事务链；二是 postmerge v3 呈拍页把 packet v9 的 disposable push ref 与 mainline promote ref 改错，且 live remote 上 `main=413a74b3...` 已不满足页内 `origin/main=371f25fb...` 前置。后续 v9.1.2 corrected terminal receipt、AMENDMENT-2、postmerge v4/v5/v6/v7 与 sentinel 修复逐轮闭合，ULTIMATE-DELTA-6 才升 `CLEAR_FOR_KEYS`。机理是本地门绿、READY 页、REPORTS-INDEX、STATUS-BOARD 和 watcher 都不是 push/promote authority：终态 truth 必须由脚本化事务链 + raw transcript/state/receipt + producer≠auditor 审计闭合；键页 READY 必须做 packet↔page↔live-ref 三方 join，并保证呈拍消息 anchoring 与页内 anchoring 一致；remote rerun 必须用集合差/唯一 run/exact-SHA/job 断言 fail-closed；sentinel 只能作 observation/wakeup 辅助，不能替代 exact key sentence、readback 和 receipt。边界：`CLEAR_FOR_KEYS` 只表示键执行前置可消费；本 v5 增补时点 KEY3 promote 已真实发生（sentinel `PROMOTED`，mainline=`70a2884e...`），但仍不证明 remote GREEN 之外的 operator-pass、V-PASS、C6 acceptance、candidate、mobile/true-device 或 live_api。锚 `runs/2026-07-12-ma18/reports/ULTIMATE-PREPUSH-AUDIT-by-w5.md:20-27,43-57,59-77` + `runs/2026-07-12-ma18/reports/INCIDENT-LOG-ma18-v2-by-w7.md:120-150,168-188` + `runs/2026-07-12-ma18/receipts/AMENDMENT-2-key-receipts-join.md:3-18,20-29` + `runs/2026-07-12-ma18/reports/ULTIMATE-DELTA-6-by-w5.md:17-21,25-31,52` + live `git rev-parse HEAD`=`70a2884e797bb3a35fc6ba912a6f7fa8fd027d87` + `sentinel-state`(stage=PROMOTED)。

## M.92 多轮 delta 收敛审计：可数缺陷逐轮修，归零后才停损开放负例（2026-07-13 ma18 后半场，N2-①）
ma18 postmerge 三键页经 ULTIMATE 换面终审起手 `DO_NOT_PUSH`（P0×2），再经六轮 delta（PREPUSH→RECHECK→delta-3…delta-6）收敛到 `CLEAR_FOR_KEYS`：每轮 finding 都能落到「改哪一文件、哪一负例 fixture、哪条 exact 句」，属 verification-economics「可数维度补全可修」侧，不是无限语法追逐。全程未因「审太久」中途停损（§23.6 / `LEIGE-DIRECTIVE-continue-cornercase.md:7` 只收有 24h actionable consumer 的 corner，前四轮 finding 全 actionable → 不触发停损是正确应用）。门绿≠属性成立再实证：sentinel 在 forged-pair/missing-TO 上多次「case 看起来绿、属性仍 fail-open」，必须 full negative fixture 每轮重跑，不能只修上轮点名的 case。停损正确落点：CLEAR_FOR_KEYS 后声明——再提 observation-only 新负例类且无 consumer → PARK/non-blocking；执行 enforcement 归 exact 句 + fail-closed readback + receipt，watcher 不升 authority。范式：①delta 链模板 `PREPUSH register → 每轮发现→修复sha→独立delta复核→verdict`；②停损二分门（可数+可复现+有修法→禁疲劳停损；开放假设/无24h consumer→必须PARK）；③负例全量重跑纪律。锚 `runs/2026-07-12-ma18/reports/ULTIMATE-SAGA-CHAIN-ma18-by-w7.md` §1六轮表+§2停损注记 + `runs/2026-07-12-ma18/reports/ULTIMATE-DELTA-6-by-w5.md`(verdict=CLEAR_FOR_KEYS, stop-loss AGREED) + `runs/2026-07-12-ma18/reports/ULTIMATE-PREPUSH-AUDIT-by-w5.md`(R0 DO_NOT_PUSH) + `runs/2026-07-12-ma18/receipts/LEIGE-DIRECTIVE-continue-cornercase.md:7` + 关联 M.61/M.62。

## M.93 哨兵是「门保护系统」，负例自测保护门：上线 watchdog 先负例包自测再挂 live（2026-07-13 ma18 后半场，N2-②）
ma18 哨兵从「轮询新文件+HEAD钉死」演进到 v6 级 fail-closed 三态机（live `sentinel.sh`；`sentinel-state` 现态 `PROMOTED` pair `371f25fb…→70a2884e…`）：交付面 anchored exact-path ignore（绝对路径禁 substring 误吞）；状态机 `SEALED|PROMOTE_WINDOW|PROMOTED` 三态，promote 中间态容双头；pair 写死 `APPROVED_FROM/TO`，`PAIR_OK` 要求 FROM/TO 双 exact（删 missing-TO fail-open）；`%0` composer 卡 `REPORT|/BLOCKED|` → 自动 Enter+日志自愈。机理：哨兵是 promote/污染的廉价到达点，但若门本身 fail-open 会系统性放行假绿 HEAD；属性（exact pair/非空TO/合法stage）必须用静态 fixture 负例证明 REJECT，不能只证明 happy path ACCEPT；PROMOTE_WINDOW 比单钉 HEAD 更贴真实 promote 时序；worker 少按 Enter 导致回写卡输入框是蜂群高频故障，哨兵侧自愈把「送达硬门」从自觉下沉到机械。范式：①哨兵能力成熟度模型 L1→L5；②「门保护系统」配对口诀=上线任何 watchdog 先负例包自测再挂 live；③ignore 列表 SSOT 绝对路径一行一个，改 ignore=显式 commit 事件。锚 live `sentinel.sh` / `sentinel-state` / `sentinel-ignore` + `runs/2026-07-12-ma18/reports/ULTIMATE-DELTA-5/6-by-w5.md`(BLOCKER-03) + `runs/2026-07-12-ma18/receipts/postmerge-keys/POSTMERGE-3KEYS-EXEC-RECEIPT.md` + 对照 w4③。

## M.94 远程动作网络瞬断属 EXPECTED_TRANSIENT：有界重试+每 attempt 落盘+终态 readback（2026-07-13 ma18 后半场，N2-③）
ma18 三键批亲执行中同构网络瞬断×2 均未翻车：键2 `gh` dispatch `unexpected EOF` → 有界重试建 run `29197406884` 唯一新建、watch 至 exact-SHA GREEN（`KEY2-dispatch.out`）；键3 `git push` `Recv failure: Operation timed out` → attempt 2 成功 `371f25fb..70a2884e`（`KEY3-push.out`）。终账 `POSTMERGE-3KEYS-EXEC-RECEIPT.md` 两键均 GREEN，显式写「网络瞬断×2 均有界重试闭合，全程 receipt 化（P0-01 定式执行层实践）」。机理：P0-01 终态/执行路径禁事后升格，失败必须留可审计中间态（attempt 编号、stderr、retry rc），成功以 readback exact 为准；网络瞬断是执行层常态噪声非合同缺陷，合同层应预置有界重试+每次 attempt 落盘+终态断言；区分合同错（假 DONE/错 ref→改页改脚本）与瞬断（重试矩阵不改语义）。范式：①远程动作 retry matrix（dispatch/push/api：max N、退避、每 attempt 写 .out、终成功必须 readback）；②呈拍页预写「网络瞬断属 EXPECTED_TRANSIENT 走有界重试不走 UNEXPECTED_RED 停门」。锚 `runs/2026-07-12-ma18/receipts/postmerge-keys/KEY2-dispatch.out` + `KEY3-push.out` + `POSTMERGE-3KEYS-EXEC-RECEIPT.md` + ULTIMATE R0 P0-01。

## M.95 数字可质询：键账=count(独立 RATIFY 键) 附枚举表，弱席也须质疑 commander 口数（2026-07-13 ma18 后半场，N2-④）
ma18 键数曾口述「11 键」，w6 强席质询后 standing auth 更正为实账 `14 键 / 5 RATIFY receipt`：`V8(1)+KEY-PUSH@371f25fb(1)+B1(2)+W-P1 merge批(7)+postmerge(3)`，辅助件（AUTO-BATCH×3、RISK-ACK×2、SKILL-RULING）不计键；`%13` 是独立 codex(gpt-5.6-sol xhigh) 席，阵容=3 grok(`%9/%10/%11`)+1 codex(`%13`)+2 hermes(`%12/%14`)（commander 澄清回执已纠正早期 w6 误读「%13=MAformac node」）；误把 node/idle shell 算进 worker 分母会污染「拉满 N 席/N 键」账。机理：commander 有编排权威，数字出口易被 worker 礼貌性照收（authority halo，对照 M.84）；「11」是派生聚合口误，「14」是 RATIFY 收据枚举，claim-vs-reality=聚合口误 vs 收据加法；强席质询价值=不口头认数逼 commander 把分母写进 standing auth/receipt 而非会话记忆，且弱席也该质疑（质询权不按模型强弱分配）。范式：①键账 SSOT 公式 `键数=count(独立 RATIFY/呈拍键)` 附枚举表，辅助 ACK/ruling 显式排除；②派单写「任意 worker 发现 commander 数字与 receipt 加法不一致→必须 BLOCKED/质询不算越权」；③派单前 `tmux list-panes`+角色表，禁把 node 算进 worker 分母。锚 `runs/2026-07-12-ma18/receipts/STANDING-AUTH-fullauto-starred.md`(§键数勘误11→14) + `runs/2026-07-12-ma18/reports/MA19-BOOTSTRAP-FINAL-by-w6.md`(pane 实测) + 对照 w4①/D-089。

## M.96 外源复盘走「处方吸收链」：异源终审四枚举 ADOPT/REJECT/GRILL + 决策链 recall + 滚动台账（2026-07-13 ma18 后半场，N2-⑤，HY3 复盘采纳项）
HY3 外源复盘（`MAformac/reports/retrospective-maformac-2026-07-12/` 三轮）进入 ma18 后未照单全收，走完整吸收链：`hy3 三轮复盘原文 → w2 处方层对抗终审 XAUDIT-RETRO-PRESCRIPTIONS（ADOPT/ADOPT_WITH_FIX/CONFLICT_NEEDS_GRILL/REJECT）→ commander RETRO-ADOPTION-LEDGER 滚动台账（逐处方吸收动作+状态+事实纠错）→ 各 worker 线落地`。关键裁决：demo 进度仪表非阻断→ADOPT(+三层字段fix，禁单数「可演」)；S8 以1场景为唯一验收→REJECT(硬冲 D-114 五布尔+S9→S10 链)；P0 单场景 scope 收缩→CONFLICT_NEEDS_GRILL(demo surface vs 契约 SSOT 须分层)；preset 授权→ADOPT_WITH_FIX(禁重复发明 D-025)；复盘双栏评分模板→ADOPT 并入 KEY-DOCS FINAL-v2；文档够用阈值→ADOPT_WITH_FIX(只限 T5/非 load-bearing)。吸收时更正复盘 stale claim（S8「从未点火」→D-120 曾真点火未收口；T04a 已 merge）。机理：外源复盘=单 frame 草稿（worker-output-adversarial 规则对象），处方层必须决策链 recall 否则重发明已锁授权或轴混淆（S8 训练轴 vs 客户可演轴）；三分枚举强制「不能全是 ADOPT」——REJECT/GRILL 是一等公民；台账滚动把「审计报告完成」转成「吸收状态可扫」防 completion bias；事实纠错与处方裁决同表避免 ledger 与 reality 分叉。范式：①复盘吸收四步模板 `外源复盘→异源处方终审(四枚举)→决策链 recall 表→滚动台账+落地 owner`；②终审强制列每处方「冲突的 D-xxx/lessons」，空列=审计未做 recall→REQUEST_CHANGES；③台账完成判据=处方全 ✅ 或 key-ready/BALLOT 态而非「报告已写」。锚 `runs/2026-07-12-ma18/reports/XAUDIT-RETRO-PRESCRIPTIONS-by-w2.md`(46609e4a…) + `runs/2026-07-12-ma18/reports/RETRO-ADOPTION-LEDGER-by-commander.md`(c67d3903…) + 原件 `MAformac/reports/retrospective-maformac-2026-07-12/` + `runs/2026-07-12-ma18/reports/DRAFT-LEARN-EVAL-ma18-night2-by-w1.md`(N2-①…⑤)。

## M.97 复盘评分拆「项目交付分 / 组织资产分」两栏（90 天窗口）（2026-07-13 ma18，HY3 复盘采纳 #6，磊哥已批）
HY3 复盘整改处方 #6（`XAUDIT-RETRO-PRESCRIPTIONS-by-w2.md:72` R4 → `RETRO-ADOPTION-LEDGER-by-commander.md` #6 ADOPT）：评分拆「项目交付分 / 组织资产分」两栏，90 天窗口。机理：收益计量若把治理资产只算过程（R4 已立「组织资产」=lessons M.61–M.89 门形态），会让「写了多少文档」淹没「资产是否真沉淀」；两栏分离使交付分（本次可演/代码/验收）与资产分（可复用 lessons/决策链/对抗审模板）独立可评，避免 completion bias 把「报告已写」当「资产已落」。边界：纯复盘模板、零决策冲突；落 docs 仅作 lessons 候选，不碰产品 SSOT、不授权任何键/merge/S8。锚 `runs/2026-07-12-ma18/reports/XAUDIT-RETRO-PRESCRIPTIONS-by-w2.md`(46609e4a…, #6 行) + `runs/2026-07-12-ma18/reports/RETRO-ADOPTION-LEDGER-by-commander.md`(#6 ADOPT) + 对照 M.61–M.89 资产链。

## M.98 「文档够用」停止阈值：90 天窗口，只限 T5 历史档与非 load-bearing 级联（2026-07-13 ma18，HY3 复盘采纳 #7，磊哥已批）
HY3 复盘整改处方 #7（`XAUDIT-RETRO-PRESCRIPTIONS-by-w2.md:73` R1 → `RETRO-ADOPTION-LEDGER-by-commander.md` #7 ADOPT_WITH_FIX）：定义「文档够用」停止阈值，90 天窗口。Fix（w2 终审）：停止阈值**不得取消 T0 活基线级联**（doc-cascade T0–T3 锁活基线/验收链），**只限 T5 历史档 + 非 load-bearing 级联**；对标 M.44 fresh inventory「该写才写」非「少写文档」。机理：文档先行是宪法（基建文档级联），但「够用即停」需有硬边界——无限补文档是 completion bias 的另一面；阈值把「非承载级联的历史档」从「必须补全」降级为「90 天内补齐即可」，而承载活基线的 T0–T3 永远不在「够用」豁免内。边界：与宪法「文档先行/基建文档级联」**存在摩擦**但非硬锁（w2 终审判定 R1）；本 lessons 只立阈值草案，不砍 T0–T3、不取消活基线级联。锚 `runs/2026-07-12-ma18/reports/XAUDIT-RETRO-PRESCRIPTIONS-by-w2.md`(46609e4a…, #7 行 + Fix) + `runs/2026-07-12-ma18/reports/RETRO-ADOPTION-LEDGER-by-commander.md`(#7 ADOPT_WITH_FIX) + 对照 M.44 fresh inventory / doc-cascade T0–T3。
