...
**目标**:Mac 上跑通"文本 → 意图 → mock 车控 → 卡片亮 + 文字回复"闭环。**不接 ASR(D15 文本先行)、不接 MCP、不 LoRA、不 barge-in。**
...
- **instructor → 类型接住模型输出**:Swift `Codable+enum+validator`,参数范围硬校验(温度 16-30 / 窗 0-100 / 座椅 0-3 / 区域枚举不可造);越界不修正不猜,转澄清/拒绝。强化 D5。
- **vss-tools → 开发期生成器**:`capabilities.yaml`(源)→ 生成 `VehicleCapability.swift` + `tool_schemas.json` + 能力表.md(一处定义三处生成 + 规格漂移检查)。明确进 B 类。
