...
status: ✅ 视觉门 grill 收口（C0/C1 校准 + Q1 范围 + U32 框架 + U33/U34/U35/U36/U37 全拍，2026-06-26）；待落 = B 流程回写 plan/skill + C 代码 + OpenSpec change propose；设备：仿真 iPhone 17 Pro/Pro Max（主验收），真机 iPhone 15 Pro Max（延后不急）
date: 2026-06-26
...
> ⚠️ **设计态 ≠ 执行态（completion-claim 总边注）**：本档拍的是【设计决策】；A-2 仍 **PARTIAL**，**8.A/8.C2 未关**（`docs/CURRENT.md:85`）。U33/U37 的 C 层代码**未落完**（见各段代码态边注），不得因 grill 拍板写成视觉验收闭合 / 代码已实装。
...
### U33 — L1 zone_compare 语义降级 + long-run stop-rule（✅ 设计拍定，⚠️ C 层代码未落）
...
- ⚠️ **代码态（completion-claim）**：`Tools/checks/phase2_zone_compare.py:87` 仍是 `def rmse(...)` RMSE 计算 + TSV 输出，**未改 PASS/WARN/FAIL stop-rule** → U33 是设计决策，C 层实装落「ABC 一个 change」的 C 部分，**待落**。
...
- [ ] **apply 待做（后续）**：8.G3 plan v3 heavy-work + `heavy-work` skill 回写（B 流程）/ 8.G4-G9 C 代码实装。
- [ ] `openspec validate --strict ui-presentation` + `make verify`（amend 后机械门）。
