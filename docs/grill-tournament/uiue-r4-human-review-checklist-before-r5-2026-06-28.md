...
|---|---|---|---|---|---|---|---|---|---|
| HR-01-mainline-bridge-authority | `/Users/wanglei/workspace/MAformac/docs/project/phase0/uiue-r4-mainline-coauthor-receipt-2026-06-28.md` | shared bridge authority 仍未被 mainline owner ratify | mainline `docs/CURRENT.md`; Step 1 receipt; Step 4 ledger | `adopt_uiue_bridge` / `create_mainline_visible_carrier_with_mapping` / `reject_for_now` | `create_mainline_visible_carrier_with_mapping` accepted by human review | C01/C03 可从 deferred 进入 accepted route after mainline carrier lands | R5 保持 blocked | yes until mainline carrier lands | UIUE docs are not mainline acceptance |
| HR-02-scope-origin-missing | `/Users/wanglei/workspace/MAformac/docs/project/phase0/uiue-r4-mainline-coauthor-receipt-2026-06-28.md` | `scope_origin=missing` 仍是 candidate，不是 Core 真值 | mainline `Core/Execution/ScopeResolution.swift`; Step 1 receipt | `extend_core` / `presentation_only_enum` / `remove_missing_use_fail_reason` / `defer_to_later` | no Core enum expansion; use `remove_missing_use_fail_reason`, with UI-local `presentation_only_enum` only if needed | C06 可进入明确 implementation route after mainline wording lands | C06 持续阻断 R5 | yes until mainline wording lands | no claim that Core `missing` is accepted |
| HR-03-mainline-roadmap-alignment | `/Users/wanglei/workspace/MAformac/docs/project/phase0/uiue-r4-mainline-coauthor-receipt-2026-06-28.md` | mainline route board 仍是 `not_proposed`，需 owner 明确是否允许 R5 phase 开始 | mainline `docs/CURRENT.md`; Step 1 receipt | `allow_after_bridge_owner_receipt` / `keep_blocked` | `allow_after_bridge_owner_receipt` accepted by human review | 可解锁 C18 after bridge owner receipt exists | R5 继续 blocked | yes until bridge owner receipt exists | no claim that route board already moved |
