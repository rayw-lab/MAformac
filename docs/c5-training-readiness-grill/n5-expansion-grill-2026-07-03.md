...
|---|---|---|---|
| N5E-001 | **C 混合形态**：controller 从 locked quota 出批次 manifest；4 条 sub-CC lane 并发；🔴 **warmup 首批 N=50**（canary 60 行已证该量级可控），微批 D9/A12 全 PASS 后升到 EXPANSION-PLAN §0 的每批 75 行（run 目录 EXPANSION-PLAN-draft.md，%45 实算）；warmup KPI 实值见 BATCH-CONTRACT-rev2 §2（warmup_rows≥100/warmup_all_active_pass≥0.90/warmup_D9_A12_structural_fail=0/controller_merge_time_per_batch 记录），green-expected-bad 三情景 | CG44-P1-4（N 条件化）+P1-5（三情景+warmup KPI） | default_locked |
...
| N5E-004 | judge 抽样：**机械可判维（D5 泄漏/D6 脱敏/D9 ledger 对账）下沉脚本全量跑；LLM judge 只审语义维（D1/D2/D8 等）按 family 抽样 min(50,max(20,10%))**；可执行 family_judge_status 状态机+Wilson 下界+next-action；warning 行拆结构性/语义性两类，硬性上限+超限升级而非无界全审；🔴 抽样分母唯一定义=controller 汇总边界的 Gate7 family accepted_candidate_pool，receipt 必带 `candidate_count_denominator/candidate_pool_sha256/sample_row_ids/sample_size_formula_version` 四字段（CG45-P1-1，字段定义随 DIVERSITY/WAVE rev2 §4.5） | CG44-P1-1（状态机可执行化）+P1-2（warning 行拆分封顶）+P1-3（owner/tool 矩阵）+CG45-P1-1（分母+receipt 字段） | default_locked |
| **N5E-005** | 🔴 **磊哥键**：人工精度门 staffing——⭐磊哥本人按 sample size 抽检（拍点5 原 default）或拍「首波跳过人工门，数据门+judge 双机械门先行」 | — | **pending_leige** |
| **N5E-006** | 🔴 **磊哥键（卡 merge 链）**：扩量启动前 pin merge 后 main SHA；controller 记录四元组（main_pin_sha/quota-config SHA/catalog-manifest SHA/vendor enum）；中途 main 变=旧批降级 candidate 不混正式 receipt | EXPANSION-PLAN §1 全文吸收 | **pending_leige（merge 后变为可机械校验；pin receipt+pinned main 重跑门缺任一=未满足）** |
...
| R2 评审体系（抽样/声称分层/人工门） | N5E-004 与 N5E-012 + CG44 P1 第1、2、3条 + CG45 P0-1、P1-1（分母）、P1-2、P1-4；N5E-005 磊哥键 | default_locked（005 除外） |
...
| JUDGE-SAMPLING rev2 → 可执行状态机（%43，已交）+ 🔴 补丁：并入 CG45-P1-1 分母与四 receipt 字段（复制 DIVERSITY/WAVE rev2 §4.5 定义） | N5E-004 与 N5E-012 | 🔄 补丁在途 |
| canary 三件套+INDEX 归档 | N5E-010 | 🔄 %43/%44 在产 |
| 磊哥键打包 | N5E-005（人工门）+ N5E-006（基座 pin：merge 后可机械校验，pin receipt+重跑门缺一不算满足） | ⬜ 与 billing/merge/run-auth 同包上抛 |
