...
本稿只回答 D5/D7/D8 的 10 个子点，全部 `proposed`，待 commander/磊哥收口。已承接：E-lite 方向、8K 预算待拍、gate3 六轴同源、L1 规则快路不碰模型、Phase-1 只做 construction（manifest/codegen/训练与 C6 侧同源/digest receipt），不做 runtime 预路由实装。
