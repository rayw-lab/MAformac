...
> **2026-07-05 晚 formal-launch warning**：本文的 `f044_watchdog.py` / WD-15 属 F044 shorttrain 历史执行环境，不是 formal 1800 watchdog surface。Formal 1800 已由 W-G2/W-H2 收敛到 `watchdog-command-template.sh` + `formal-watchdog-draft.py`；若 formal command 再指 shorttrain `f044_watchdog.py`，按 W-G2/W-H2/W-J 口径为 HOLD。当前 formal launch 仍因 host gate HOLD 未启动，watchdog 也未 armed。
