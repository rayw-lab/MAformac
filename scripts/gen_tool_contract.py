...
def canonical_ir_map_text(ir_map):
    return json.dumps(ir_map, ensure_ascii=False, sort_keys=True, separators=(",", ":")) + "\n"


def write_swift_ir_map(path, ir_map):
    fingerprint = hashlib.sha256(canonical_ir_map_text(ir_map).encode("utf-8")).hexdigest()
    lines = [
        "// Generated by scripts/gen_tool_contract.py. Do not edit by hand.",
        "",
        "public enum DDomainIRMap {",
        f"    public static let irMapCompiledFingerprint = \"{fingerprint}\"",
        "    public static let irMapCompiled: [String: DDomainIRMapEntry] = [",
    ]
    for name, entry in sorted(ir_map.items()):
        lines.append(
            "        "
            f"{swift_string(name)}: DDomainIRMapEntry("
            f"device: {swift_string(entry['device'])}, "
            f"irPrimitives: {swift_string_array(entry['ir_primitives'])}, "
            f"valueTypes: {swift_string_array(entry['value_types'])}"
            "),"
        )
    lines.extend([
        "    ]",
...
def main():
    ...
    if args.scope in ("demo", "both"):
        ir_map = build_ir_map(demo_tools)
        write_json(output_dir / "D_domain.tools.demo.json", demo_tools)
        write_json(output_dir / "d_domain_ir_map.json", ir_map)
        write_swift_ir_map(Path("Core/Contracts/DDomainIRMap.generated.swift"), ir_map)
