...
verify: .venv/.deps.stamp verify-source regen verify-refs verify-cross-section verify-surface verify-c6-shape verify-default-scope verify-register verify-c1-matrix verify-c1-fallback verify-c1-s10 diff test verify-contentview-wiring
...
verify-c1-matrix:
	...
	elif [ -z "$(A1_MATRIX_MANIFEST)" ]; then \
		echo "SKIP_RAW_DEPENDENT verify-c1-matrix (set A1_MATRIX_MANIFEST)"; \
	else \
		mkdir -p /tmp/maformac-c1-checks; \
		$(PYTHON_BOOTSTRAP) Tools/checks/check_capability_matrix.py check \
			--manifest "$(A1_MATRIX_MANIFEST)" \
			--matrix contracts/demo-capability-matrix.json \
			--receipt /tmp/maformac-c1-checks/capability-matrix.json; \
	fi
