...
- **Line B / S9 holdout**：D-122 Opus raw 生成后，J1 全量判 47/64 PASS，触发 D-124 修复战役。D-124 已拍：near-dup 三方合成剔 10 真撞、救 3 FP，register 用既有 `imperative` + `register_subtype` 修 4 行，补 10 行 per-bucket。v2/v3 修复链正在推进，J2/JFX/OPUS-H3 等 report 已落，但 **holdout v3 全量 judge + sha freeze 仍在途**；进 composite manifest 前不得写 holdout ready。
...
2. **holdout v3 判定链**：补行/修复后做全量 blind judge、near-dup 全局复核、sha freeze、canonical 联动；未完成前 S9 manifest 不得消费。
...
- S9/S9b/S10 未执行；holdout v3 未全量 judge/freeze。
