...
## 9. 当前路由：V9 fan-in 技术收口（2026-07-14）

> 当前技术 subject 从 `a3eaca03174f3266a2d52645648cadcfe5da8bb8` 的严格终审 `PARTIAL` 出发，已完成 bounded identity repair：canonical reanchor 对 registry digest / registry file SHA / B7 envelope / V1 authority source members / V1 native candidate receipt / V1 ratification packet 做事务性级联，并把 five authority-eval suites + B7/V1 direct checker 接入 `make verify-all` 与 `make verify-ci`。最终 commit/remote SHA 读取 `docs/handoffs/2026-07-14-v9-fanin-identity-repair.md` 与 live Git，不从本段猜测。
>
> **技术 DONE 边界**：只恢复 A/fan-in identity + aggregate gate 的机械可证 DONE。S8、真实三臂、B7 freeze ceremony、V1 ratification ceremony、operator/V-PASS、C6 acceptance、candidate signed，以及 B lane deferred lifecycle/App/customer-shaped runtime 均未完成、不得翻格。
>
> 冷启动顺序：`docs/CURRENT.md` 顶部 V9 段 → 最新 handoff → live Git/remote → 本轮 closeout。下方 roadmap v5/daywork 内容为历史推进背景，与本段冲突时按历史处理。

## 【历史】roadmap v5 daywork 现态（C6 done 起点 -> macOS app 全功能闭环）

> **当前推进事实源 = `active_baseline_v6`：`docs/roadmap-2026-07-11-v6-closure-baseline.md`（RATIFIED，D-142，`baseline_activation=PENDING_CASCADE`）+ `docs/commander-log/decisions.md` D-133~D-142 + `/Users/wanglei/Projects/agent-tmux-stack-research/runs/2026-07-11-ma14/`；v5 roadmap 转 historical。**
>
...
3. `docs/roadmap-2026-07-11-v6-closure-baseline.md`（RATIFIED，D-142，`baseline_activation=PENDING_CASCADE`；v5 roadmap historical）。
...
This project is indexed by GitNexus as **MAformac-r5-main-current** (39744 symbols, 98982 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
