...
>
> 🔴 **Post-A2 default_scope 前置 blocker(2026-06-24,G01-G28 已拍)**:A2 已合 main 后,下一步不是直接 retrain-c5 / rebuild-c6 / demo-golden-run,而是先起 `define-demo-default-scope` Phase -1 OpenSpec carrier。它必须把 C2 `default_scope`、omitted/explicit/fan-out、collection aliases、legacy UI key disposition、scope-origin presentation、C5/C6/golden/UIUE 依赖写入 proposal/design/tasks/spec。权威决策包 = `docs/grill-tournament/demo-default-scope-grill-decisions-2026-06-24.md`;执行计划 = `docs/superpowers/plans/2026-06-24-phase0-d1-d10-openspec-gates.md` 的 Phase -1。未完成前不得启动训练、C6 重建验收、真实评测、voice 或 demo-golden-run。
>
