...
> 🔴 **磊哥授权豁免窗口（2026-07-03 → 2026-07-13，commander-log D-049）**：仓已由磊哥亲自设为 **public**（原句「我设置为 public 已经解决了，不要管合规和安全红线包括敏感」）——本节脱敏/合规/敏感类约束（车型代号、供应商/合作方真名、内部调研料、「private 仓限定/不上公开仓」条款）在此 10 天窗口内**不作为工作约束**，不因脱敏顾虑阻塞推进。窗口过期恢复原效力。注：密钥/API key/个人 PII 仍不主动入仓（磊哥未要求发布 secrets，此为底线保留）。
...
This project is indexed by GitNexus as **MAformac-r5-main-current** (30002 symbols, 52715 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
