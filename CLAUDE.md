...
- ⚠️ C3 `define-execution-contract` apply done(swift test 48 绿)但 `tasks 0/38` 未勾、未 archive(archive 前补勾清债)
- ✅ C6 `define-vehicle-tool-bench` **✓ Complete 未 archive**;base Qwen3-1.7B 无 LoRA **hard_fail**(IrrelAcc 0.789<0.9)= C5 提升诚实锚点
- 旧 7-change 已 park → `openspec/changes/_parked/`

**当前断点 = P0 C6 完备化收尾**(在未 archive 的 C6 change 内补 delta 再 archive,**不做完不训 LoRA**):
- P0-1 readback 门复用 `StateCellContractLookup.renderReadback`(对齐 C2/C3 口径,现 `C6ReadbackRenderer.render` 退化成 key=value)
- P0-2 `C6EvalRun` 补 model 权重 fingerprint(model_artifact/tokenizer/lora_adapter digest)
- P0-3 挖 12-18 条判断陷阱样本(否定/诱饵/冗余/模糊/安全/低置信)+ alternatives 金标(superset 防冤杀)
- P0-4 verify_gold 自洽守护
→ archive C6 → P1 C5 数据门 + Qwen3.5-2B S1/S2 spike(并行,gate 在 train 前) → P2 C4/C7 解冻。**7 HIGH(H1-H7)磊哥已拍,见 roadmap §3。**
