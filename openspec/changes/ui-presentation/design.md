...
- **AD-8.1 默认主驾不澄清**（D8.1）：区域 scope 用户未指定 → 卡片**默认锚定默认 scope 态**（座位→主驾 / 屏类→中控屏 / 前后→前·前排），渲染 satisfied/changing，**不渲「请选区域」空态、不弹 clarify 打断**。⚠️ scope 默认值【填槽执行逻辑】= NLU/Core（**非 UIUE**），UIUE 只展示默认态。10 族 scope 核源 = `main:contracts/state-cells.yaml`（空调温度/风量·座椅加热/通风/靠背·车窗 = 座位 scope）。
...
- **AD-8.4 触摸 = 极简查看**（D8.4）：卡片**只读状态展示（非手动控制器）**，点卡→高亮+tooltip，无调节逻辑。demo 语音主线，触摸防"死图片"。🟡待磊哥确认现场戳卡习惯。
