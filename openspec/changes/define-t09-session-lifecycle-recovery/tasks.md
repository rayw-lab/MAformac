...
- [ ] Create the W8 carrier files and keep the capability id as session-lifecycle.
...
- [ ] Expand the canonical seed into the complete ADDED Requirements and GIVEN/WHEN/THEN scenarios.
...
- [ ] Run openspec validate --strict for the change and keep structure incomplete if it is not rc0.
...
- [ ] Define the observable session and generation identity contract.
...
- [ ] Freeze the legal transition and terminal disposition semantics.
...
- [ ] Define the single-owner publication boundary.
...
- [ ] Define compound request deterministic ordering.
...
- [ ] Define last reconciled stable checkpoint as the only recovery source.
...
- [ ] Define terminal/checkpoint/child-fence join.
  - Output: join matrix.
  - Acceptance: recoveryReady requires all join members.

## S6 — RecoveryReady and new generation

- [ ] Define recoveryReady and new-generation publication.
...
- [ ] Define old-generation fence after recovery.
...
- [ ] Preserve the W10 TTS and presentation boundary.
...
- [ ] Keep accepted, refused, cancelled, unsupported, timeout, and error outcomes distinct.
...
- [ ] Define 010a deterministic interleaving profile receipt.
...
- [ ] Define 010b recipe provenance and claim ceiling.
...
- [ ] Keep planned gate fields complete.
...
- [ ] Keep future App seam, checker, negative suite, Makefile wiring, materialization writer, and real-process runner as future coding work.
  - Output: implementation-ready task slices only.
  - Acceptance: no Swift/Python implementation is added by this carrier writeback.
- [ ] Define source gate wiring as future verify and verify-ci work, and exit gate wiring as future verify-wave2-runtime-only work.
  - Output: planned gate recipe.
  - Acceptance: exit gate is not placed in source-free verify-ci.
- [ ] Record GitNexus impact/risk-ack and later coding key as separate future authorization.
...
- [ ] Write the current-day pair receipt with plan path/SHA, carrier path, every change file/SHA, repo HEAD, paired_at, paired_by, conventions SHA, and strict rc.
  - Acceptance: receipt is complete and independently hashable.
- [ ] Flip the W8 plan frontmatter to SUPERSEDED_BY_CARRIER only after pair receipt is complete.
