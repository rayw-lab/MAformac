...
- 新增 `demo-capability-governance`：规定 120 格 matrix 守恒、逐格 basis、`canDemo` 四件套、`primary_class`、closed fallback enum/catalog、10 族×4 reason coverage、CG-036 治理条件、probe/no-mutation 门、S10/mounted expansion 与 rollback 红线。
...
- 不由 C1 授权 mounted 1→N；不执行 S9/S10，不把 prelay、FastPath alias 或 conditional lane 写成 default `canDemo`。
