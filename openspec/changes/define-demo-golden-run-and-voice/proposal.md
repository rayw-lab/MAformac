...
本 change = A2 6 步依赖序末端 + demo 交付层，依赖 `migrate-d-domain-tool-surface`（D-domain 工具名）+ `rebuild-c6-four-layer-bench`（must_pass + c6_case_id_derived 关联）。新建两个 capability：`demo-golden-run`（炸场合同回放 SSOT）+ `voice-pipeline`（ASR/TTS/VAD 链路契约）。
