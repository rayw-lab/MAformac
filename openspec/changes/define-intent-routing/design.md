...
**ContextRewriter(短期记忆=上下文改写,源料抽象,见 baseline D38/§18)**:带指代 query 改写成上下文无关完整 query 再走单轮抽槽(把 multi-turn 16.88% 降维成 single-turn)——6 规则:①实体双源(query/answer,两者都有优先 answer)②白名单继承(只继承实体类:影视/歌曲/poi,描述类不继承防污染)③序号指代(answer 序号→实体)④跨轮回溯(显性指代+跨越轮无歧义)⑤相对量读端态(同上)⑥「不改写」也是决策(对已生成结果的编辑不改写)。每继承槽带 `slot_source` + 音区 hard gate(跨音区不继承)。**不塞 raw 对话历史进 context**(三方印证:真实座舱 rewrite / On-Device CSO 降10-25x / change3 不存原样历史)。
